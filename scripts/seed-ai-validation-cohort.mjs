...
const profiles = [
  { key:'yui',email:'verify-ai-yui@hangoutnow.example',name:'ユイ（AI検証）',phone:'+819022220101',birthDate:'1994-05-16',gender:'FEMALE',area:'新宿',photo:'demo-mami-profile-main.jpg',interests:['カフェ','映画','英会話'],activities:['カフェ','映画'],slots:['WED','FRI','NIGHT'],urgency:'TODAY',travel:25,groups:[3,4],budget:[1000,3500],styles:['少人数','会話中心'],goals:['趣味仲間','気分転換'],first:['初心者歓迎'],alcohol:'SOMETIMES',smoking:'NON_SMOKING',avoid:['大人数'],flex:['開始時刻を調整可能'],languages:['JAPANESE','ENGLISH']},
  { key:'sota',email:'verify-ai-sota@hangoutnow.example',name:'ソウタ（AI検証）',phone:'+819022220102',birthDate:'1990-10-02',gender:'MALE',area:'渋谷',photo:'demo-host-profile.jpg',interests:['ランニング','ラーメン','写真'],activities:['ランニング','散歩'],slots:['SAT','SUN','MORNING'],urgency:'WEEKEND',travel:45,groups:[2,4,6],budget:[0,2500],styles:['アクティブ','少人数'],goals:['運動仲間','趣味仲間'],first:['経験者歓迎'],alcohol:'NONE',smoking:'NON_SMOKING',avoid:['喫煙'],flex:['雨天変更可能'],languages:['JAPANESE']},
  { key:'minji',email:'verify-ai-minji@hangoutnow.example',name:'ミンジ（AI検証）',phone:'+819022220103',birthDate:'1997-03-21',gender:'FEMALE',area:'新宿',photo:'demo-guest-profile.jpg',interests:['韓国語','カフェ','スイーツ'],activities:['カフェ','語学交流'],slots:['TUE','THU','EVENING'],urgency:'THIS_WEEK',travel:30,groups:[3,5],budget:[1500,4000],styles:['会話中心','国際交流'],goals:['語学交流','友達づくり'],first:['初参加同士'],alcohol:'NONE',smoking:'NON_SMOKING',avoid:['飲酒中心'],flex:['終了時刻を調整可能'],languages:['KOREAN','JAPANESE']},
  { key:'chen',email:'verify-ai-chen@hangoutnow.example',name:'チェン（AI検証）',phone:'+819022220104',birthDate:'1988-12-09',gender:'MALE',area:'渋谷',photo:'demo-masaya-profile.jpg',interests:['中国語','英会話','ボードゲーム'],activities:['語学交流','ゲーム'],slots:['FRI','SAT','NIGHT'],urgency:'THIS_WEEK',travel:60,groups:[4,6],budget:[1000,5000],styles:['国際交流','にぎやか'],goals:['語学交流','新しい体験'],first:['初心者歓迎'],alcohol:'SOMETIMES',smoking:'SEPARATED',avoid:['営業・勧誘'],flex:['途中参加可能'],languages:['CHINESE','ENGLISH','JAPANESE']},
  { key:'emma',email:'verify-ai-emma@hangoutnow.example',name:'エマ（AI検証）',phone:'+819022220105',birthDate:'1995-08-27',gender:'OTHER',area:'渋谷',photo:'demo-madoka-profile-main.jpg',interests:['英会話','ワイン','散歩'],activities:['英会話','散歩'],slots:['SUN','DAYTIME','EVENING'],urgency:'FLEXIBLE',travel:40,groups:[2,3,4],budget:[2000,6000],styles:['落ち着いた雰囲気','国際交流'],goals:['語学交流','気分転換'],first:['初心者歓迎'],alcohol:'YES',smoking:'NO_PREFERENCE',avoid:['騒がしい店'],flex:['場所を調整可能'],languages:['ENGLISH','JAPANESE']},
];
...
async function account(profile) {
  ...
  if (user.verificationStatus !== 'PHONE_VERIFIED') {
    const requested = await call('/users/me/phone/request', { method:'POST', body:JSON.stringify({ phone:profile.phone }) }, token);
    if (!requested.body.demoCode) throw new Error(`Demo verification unavailable for ${profile.email}`);
    user = await call('/users/me/phone/confirm', { method:'POST', body:JSON.stringify({ phone:profile.phone, code:requested.body.demoCode }) }, token).then(value => value.body);
  }

...
const publicBasePath = process.env.AIOCR_BASE_PATH?.trim().replace(/\/$/, '') || '';
if (publicBasePath && !/^\/[a-z0-9-]+$/.test(publicBasePath)) throw new Error('AIOCR_BASE_PATH must be a simple absolute path.');
const publicUrl = value => `${publicBasePath}${value === '/' ? '/' : value}`;
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/auth/login') {
      if ((!authUser || !authPassword) && db.hospitals.length === 0) return sendJson(res, 200, { ok: true, userId: 'local-user', tenantId: facilityId, role: 'ADMIN', redirect: publicUrl('/admin.html') });
      ...
      res.setHeader('Set-Cookie', `aiocr_session=${encodeURIComponent(token)}; HttpOnly; SameSite=Strict; Path=${publicBasePath || '/'}; Max-Age=${sessionTtlMs / 1000}${secureCookies ? '; Secure' : ''}`);
      return sendJson(res, 200, { ok: true, userId: authenticated.userId, tenantId: authenticated.tenantId, role: authenticated.role, hospitalName: authenticated.hospitalName, redirect: publicUrl(authenticated.role === 'ADMIN' ? '/admin.html' : '/') });
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/auth/logout') {
      ...
      res.setHeader('Set-Cookie', `aiocr_session=; HttpOnly; SameSite=Strict; Path=${publicBasePath || '/'}; Max-Age=0${secureCookies ? '; Secure' : ''}`); return sendJson(res, 200, { ok: true });
    }
    const identity = requestIdentity(req);
    if (!identity) {
      if (req.method === 'GET' && ['/login.html', '/styles.css', '/login.js'].includes(url.pathname) && await serveStatic(url.pathname, res)) return;
      if (req.method === 'GET' && url.pathname === '/') { res.writeHead(302, { Location: publicUrl('/login.html') }); return res.end(); }
      return sendJson(res, 401, { error: '認証が必要です' });
    }
    if (req.method === 'GET' && url.pathname === '/') {
      if (identity.role === 'ADMIN') { res.writeHead(302, { Location: publicUrl('/admin.html') }); return res.end(); }
      if (await serveStatic('/index.html', res)) return;
    }
    if (req.method === 'GET' && url.pathname === '/index.html' && identity.role === 'ADMIN') {
      res.writeHead(302, { Location: publicUrl('/admin.html') }); return res.end();
    }

...
type JoinRequest = { id:string; status:'PENDING'|'ACCEPTED'|'REJECTED'; message:string|null; user:{id:string;displayName:string;verification:string;profilePhoto:string|null} };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.content}>
        ...
        {screen === 'detail'&&selectedHangout&&<HangoutDetailScreen user={session.user} hangout={selectedHangout} requests={joinRequests} onBack={()=>setScreen('home')} onJoin={joinHangout} onFinish={confirmFinishHangout} onDecide={decideJoinRequest}/>}
...
function AuthScreen({ loading, error, onLogin, onRegister }: { loading: boolean; error: string; onLogin: (email:string,password:string,role?:'host'|'guest'|null)=>Promise<void>; onRegister:(input:{email:string;password:string;displayName:string;birthDate:string;gender:string})=>Promise<void> }) {
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.authPage} keyboardShouldPersistTaps="handled">
    ...
    <Image source={{uri:'https://hangoutnow-demo.onrender.com/assets/activity-photos-v1.png'}} style={styles.authPhoto}/>

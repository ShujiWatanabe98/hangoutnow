...
type Screen = 'home' | 'create' | 'detail' | 'phone' | 'chat' | 'profile';
type AlphaArea = '新宿' | '渋谷';
type JoinRequest = { id:string; status:'PENDING'|'ACCEPTED'|'WAITLISTED'|'REJECTED'|'CANCELLED'; message:string|null; user:{id:string;displayName:string;verification:string;profilePhoto:string|null} };
type CreateHangoutInput = {title:string;description:string;category:string;startInMinutes:30|60|180;publicLocationName:string;locationName:string;maxParticipants:number;area:AlphaArea};
type ReportReason='HARASSMENT'|'SPAM'|'DANGEROUS'|'SEXUAL'|'SOLICITATION'|'FRAUD'|'HATE'|'IMPERSONATION'|'OTHER';
const AREA_COORDINATES:Record<AlphaArea,{latitude:number;longitude:number}>={新宿:{latitude:35.6909,longitude:139.7003},渋谷:{latitude:35.658,longitude:139.7016}};
...
function stateLabel(hangout:Hangout){return hangout.myJoinStatus==='ACCEPTED'?'承認済み':hangout.myJoinStatus==='PENDING'?'申請中':hangout.status==='FULL'?'満員':'募集中'}
...
export default function App() {
  ...
  const loadHome = useCallback(async () => {
    ...
    void request('/analytics/events',{method:'POST',body:JSON.stringify({eventType:'DISCOVERY_VIEWED'})}).catch(()=>undefined);
  ...
  useEffect(()=>{if(session&&screen==='home'&&coordinates)void loadHome()},[coordinates,loadHome,screen,session]);
  ...
  async function joinHangout(hangout: Hangout) {
    ...
    try {
      ...
      void request('/analytics/events',{method:'POST',body:JSON.stringify({eventType:'JOIN_REQUESTED',hangoutId:hangout.id})}).catch(()=>undefined);
      await loadHome();
    } catch (cause) {
  ...
  async function openHangout(hangout:Hangout){setLoading(true);setError('');try{const detail=await request<Hangout>(`/hangouts/${hangout.id}`);void request('/analytics/events',{method:'POST',body:JSON.stringify({eventType:'HANGOUT_VIEWED',hangoutId:detail.id})}).catch(()=>undefined);setSelectedHangout(detail);if(detail.hostUserId===session?.user.id)setJoinRequests(await request<JoinRequest[]>(`/hangouts/${detail.id}/requests`));else setJoinRequests([]);setScreen('detail')}catch(cause){setError(cause instanceof Error?cause.message:'募集詳細を取得できませんでした')}finally{setLoading(false)}}
  async function createHangout(input:CreateHangoutInput){setLoading(true);setError('');try{const coordinates=AREA_COORDINATES[input.area];const created=await request<Hangout>('/hangouts',{method:'POST',body:JSON.stringify({title:input.title.trim(),description:input.description.trim()||undefined,category:input.category,serviceArea:input.area==='新宿'?'SHINJUKU':'SHIBUYA',startInMinutes:input.startInMinutes,publicLocationName:input.publicLocationName.trim(),locationName:input.locationName.trim(),latitude:coordinates.latitude,longitude:coordinates.longitude,maxParticipants:input.maxParticipants,genderRestriction:'ANY'})});void request('/analytics/events',{method:'POST',body:JSON.stringify({eventType:'HANGOUT_CREATED',hangoutId:created.id})}).catch(()=>undefined);setSelectedArea(input.area);setCoordinates(coordinates);setLocationLabel(input.area);setSelectedHangout(created);setJoinRequests([]);await loadHome();setScreen('detail')}catch(cause){setError(cause instanceof Error?cause.message:'募集を作成できませんでした')}finally{setLoading(false)}}
  async function decideJoinRequest(requestId:string,accept:boolean){if(!selectedHangout)return;setLoading(true);setError('');try{await request(`/join-requests/${requestId}/${accept?'accept':'reject'}`,{method:'POST'});setJoinRequests(await request<JoinRequest[]>(`/hangouts/${selectedHangout.id}/requests`));setSelectedHangout(await request<Hangout>(`/hangouts/${selectedHangout.id}`));await loadHome()}catch(cause){setError(cause instanceof Error?cause.message:'参加申請を更新できませんでした')}finally{setLoading(false)}}
  async function updateAttendance(status:'CONFIRMED'|'CANCELLED'){if(!selectedHangout?.myJoinRequestId)return;setLoading(true);setError('');try{await request(`/join-requests/${selectedHangout.myJoinRequestId}/attendance`,{method:'PATCH',body:JSON.stringify({status})});const updated=await request<Hangout>(`/hangouts/${selectedHangout.id}`);setSelectedHangout(updated);await loadHome();if(status==='CANCELLED')Alert.alert('キャンセルしました','参加枠を解放し、待機中の方へ空席を通知しました。')}catch(cause){setError(cause instanceof Error?cause.message:'出欠を更新できませんでした')}finally{setLoading(false)}}
  function confirmReportHost(hangout:Hangout){
    const choose=(reason:ReportReason,label:string)=>Alert.alert('通報してブロック',`${hangout.host.displayName}さんを「${label}」として運営へ通報し、今後お互いの募集とチャットを非表示にします。`,[{text:'キャンセル',style:'cancel'},{text:'通報してブロック',style:'destructive',onPress:()=>void reportHost(hangout,reason,label)}]);
    Alert.alert('通報理由を選択','緊急の危険がある場合は、アプリではなく警察・救急へ連絡してください。',[{text:'危険な行為',onPress:()=>choose('DANGEROUS','危険な行為')},{text:'迷惑行為・その他',onPress:()=>choose('OTHER','迷惑行為・その他')},{text:'キャンセル',style:'cancel'}]);
  }
  async function reportHost(hangout:Hangout,reason:ReportReason,label:string){setLoading(true);setError('');try{await request('/reports',{method:'POST',body:JSON.stringify({targetUserId:hangout.hostUserId,hangoutId:hangout.id,reason,details:`Hangout詳細画面から通報: ${label}`,blockUser:true})});setHangouts(current=>current.filter(item=>item.hostUserId!==hangout.hostUserId));setSelectedHangout(null);setScreen('home');Alert.alert('通報を受け付けました','相手をブロックし、運営の確認対象に追加しました。')}catch(cause){setError(cause instanceof Error?cause.message:'通報を送信できませんでした')}finally{setLoading(false)}}
  async function verifyPhone(phone:string,code?:string){setLoading(true);setError('');try{if(!code){await request('/users/me/phone/request',{method:'POST',body:JSON.stringify({phone})});return}const user=await request<User>('/users/me/phone/confirm',{method:'POST',body:JSON.stringify({phone,code})});setSession(current=>current?{...current,user}:current);setScreen('profile')}catch(cause){setError(cause instanceof Error?cause.message:'電話番号を確認できませんでした')}finally{setLoading(false)}}
  function chooseArea(area:AlphaArea){setSelectedArea(area);setCoordinates(AREA_COORDINATES[area]);setLocationLabel(area)}
  ...
  function logout() {
    if (Platform.OS !== 'web') void SecureStore.getItemAsync('hangout-now-push-token').then(async (token) => {
      if (!token) return;
      await request('/notifications/push-token/remove', { method: 'POST', body: JSON.stringify({ token, platform: Platform.OS }) }).catch(() => undefined);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {!selectedRoom&&['home','chat','profile'].includes(screen)&&<View style={styles.nav}>
        {([['home','ホーム'],['chat','チャット'],['profile','プロフィール']] as const).map(([value, label]) => (
          <Pressable key={value} onPress={()=>{setSelectedRoom(null);setScreen(value)}} style={styles.navItem}>
            <View style={[styles.navMark,screen===value&&styles.navMarkOn]}/><Text style={[styles.navLabel,screen===value&&styles.navOn]}>{label}</Text>
          </Pressable>
        ))}
      </View>}
...
function HomeScreen({ user, hangouts, refreshing, locationLabel, selectedArea, onArea, onLocation, onRefresh, onOpen, onCreate }: {user:User;hangouts:Hangout[];refreshing:boolean;locationLabel:string;selectedArea:AlphaArea;onArea:(area:AlphaArea)=>void;onLocation:()=>void;onRefresh:()=>void;onOpen:(hangout:Hangout)=>void;onCreate:()=>void}) {
  ...
  return <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh}/>}><View style={styles.hero}><Text style={styles.eyebrow}>{locationLabel==='エリア未設定'?(user.homeArea||locationLabel):locationLabel}</Text><Text style={styles.heroTitle}>今から{`\n`}何する？</Text><View style={styles.areaRow}>{(['新宿','渋谷'] as const).map(area=><Pressable key={area} style={[styles.areaButton,selectedArea===area&&styles.areaButtonOn]} onPress={()=>onArea(area)}><Text style={[styles.areaText,selectedArea===area&&styles.areaTextOn]}>{area}</Text></Pressable>)}</View><View style={styles.homeActions}><Pressable style={styles.locationButton} onPress={onLocation}><Text style={styles.locationText}>現在地から近い順</Text></Pressable><Pressable style={styles.createButton} onPress={onCreate}><Text style={styles.primaryText}>＋ 募集する</Text></Pressable></View></View><View style={styles.sectionHead}><Text style={styles.sectionTitle}>{selectedArea}のHangout</Text><Text style={styles.muted}>{hangouts.length}件</Text></View>{hangouts.map((hangout)=><Pressable key={hangout.id} style={styles.card} onPress={()=>onOpen(hangout)}><Image source={{uri:ACTIVITY_PHOTO_URL}} style={styles.activityPhoto} resizeMode="cover"/><View style={styles.cardTop}><View style={styles.cardCopy}><Text style={styles.cardCategory}>{hangout.category}</Text><Text style={styles.cardTitle}>{hangout.title}</Text><Text style={styles.muted}>{hangout.locationName}</Text><Text style={styles.muted}>参加 {hangout.participantCount} / {hangout.maxParticipants}人</Text></View><Text style={styles.status}>{stateLabel(hangout)}</Text></View><View style={styles.cardBottom}><View><Text style={styles.hostName}>主催：{hangout.host.displayName}{hangout.host.verification==='PHONE_VERIFIED'?' ・確認済み':''}</Text><Text style={styles.hostTier}>{hangout.host.hostStatus?.label||'ホワイト'}</Text></View><Text style={styles.detailLink}>詳細を見る ›</Text></View></Pressable>)}{!hangouts.length&&<Text style={styles.empty}>現在募集中のHangoutはありません。</Text>}</ScrollView>;
}

function CreateHangoutScreen({area,onBack,onSubmit}:{area:AlphaArea;onBack:()=>void;onSubmit:(input:CreateHangoutInput)=>void}){const [form,setForm]=useState<CreateHangoutInput>({title:'',description:'',category:'ラーメン',startInMinutes:30,publicLocationName:`${area}駅周辺`,locationName:'店名・住所を入力',maxParticipants:4,area});const valid=form.title.trim().length>0&&form.publicLocationName.trim().length>0&&form.locationName.trim().length>0;return <ScrollView contentContainerStyle={styles.formPage}><Pressable onPress={onBack}><Text style={styles.backText}>‹ ホームへ</Text></Pressable><Text style={styles.pageTitle}>Hangoutを募集</Text><Text style={styles.safetyNote}>安全のため集合場所は駅・店舗など公開された場所に限ります。店名・住所・正確な位置は承認された参加者だけに表示されます。</Text><Text style={styles.label}>公開エリア（新宿・渋谷のみ）</Text><View style={styles.areaRow}>{(['新宿','渋谷'] as const).map(item=><Pressable key={item} style={[styles.areaButton,form.area===item&&styles.areaButtonOn]} onPress={()=>setForm(v=>({...v,area:item,publicLocationName:`${item}駅周辺`}))}><Text style={[styles.areaText,form.area===item&&styles.areaTextOn]}>{item}</Text></Pressable>)}</View><Text style={styles.label}>タイトル</Text><TextInput style={styles.input} value={form.title} onChangeText={title=>setForm(v=>({...v,title}))} placeholder="例：30分後にラーメン" maxLength={80}/><Text style={styles.label}>カテゴリ</Text><View style={styles.choiceRow}>{['ラーメン','ウォーキング','ランニング'].map(category=><Pressable key={category} style={[styles.choice,form.category===category&&styles.choiceOn]} onPress={()=>setForm(v=>({...v,category}))}><Text>{category}</Text></Pressable>)}</View><Text style={styles.label}>開始まで</Text><View style={styles.choiceRow}>{([30,60,180] as const).map(minutes=><Pressable key={minutes} style={[styles.choice,form.startInMinutes===minutes&&styles.choiceOn]} onPress={()=>setForm(v=>({...v,startInMinutes:minutes}))}><Text>{minutes===180?'3時間':`${minutes}分`}</Text></Pressable>)}</View><Text style={styles.label}>承認前に表示するエリア</Text><TextInput style={styles.input} value={form.publicLocationName} onChangeText={publicLocationName=>setForm(v=>({...v,publicLocationName}))} maxLength={100}/><Text style={styles.label}>承認後に表示する店名・住所</Text><TextInput style={styles.input} value={form.locationName} onChangeText={locationName=>setForm(v=>({...v,locationName}))} maxLength={100}/><Text style={styles.label}>説明（任意）</Text><TextInput style={[styles.input,styles.multiline]} value={form.description} onChangeText={description=>setForm(v=>({...v,description}))} multiline maxLength={500}/><Pressable disabled={!valid} style={[styles.primary,!valid&&styles.disabled]} onPress={()=>onSubmit(form)}><Text style={styles.primaryText}>Hangoutを公開する</Text></Pressable></ScrollView>}

function HangoutDetailScreen({user,hangout,requests,onBack,onJoin,onFinish,onDecide,onReport,onAttendance}:{user:User;hangout:Hangout;requests:JoinRequest[];onBack:()=>void;onJoin:(hangout:Hangout)=>void;onFinish:(id:string)=>void;onDecide:(id:string,accept:boolean)=>void;onReport:(hangout:Hangout)=>void;onAttendance:(status:'CONFIRMED'|'CANCELLED')=>void}){const isHost=hangout.hostUserId===user.id;return <ScrollView contentContainerStyle={styles.formPage}><Pressable onPress={onBack}><Text style={styles.backText}>‹ 一覧へ</Text></Pressable><Image source={{uri:ACTIVITY_PHOTO_URL}} style={styles.detailPhoto} resizeMode="cover"/><Text style={styles.eyebrow}>{hangout.category} ・ {stateLabel(hangout)}</Text><Text style={styles.pageTitle}>{hangout.title}</Text><Text style={styles.detailMeta}>{new Date(hangout.startAt).toLocaleString('ja-JP')} ／ {hangout.participantCount} / {hangout.maxParticipants}人</Text><View style={styles.detailPanel}><Text style={styles.label}>集合場所</Text><Text>{hangout.locationName}</Text><Text style={styles.privacyText}>{hangout.myJoinStatus==='ACCEPTED'||isHost?'承認済みのため詳細を表示しています。':'参加承認までは、おおまかな場所だけが表示されます。'}</Text>{hangout.description&&<Text style={styles.description}>{hangout.description}</Text>}</View>{!isHost&&hangout.myJoinStatus==='ACCEPTED'&&<View style={styles.detailPanel}><Text style={styles.hostName}>{hangout.myAttendanceStatus==='CONFIRMED'?'参加予定として回答済み':'開始前の出欠確認'}</Text><Text style={styles.muted}>予定が変わった場合は早めにお知らせください。</Text><View style={styles.requestActions}><Pressable style={styles.rejectButton} onPress={()=>onAttendance('CANCELLED')}><Text>キャンセル</Text></Pressable><Pressable style={styles.acceptButton} onPress={()=>onAttendance('CONFIRMED')}><Text style={styles.primaryText}>参加する</Text></Pressable></View></View>}{!isHost&&hangout.myJoinStatus==='WAITLISTED'&&<View style={styles.detailPanel}><Text style={styles.hostName}>待機リストに登録済み</Text><Text style={styles.muted}>空席が出たら通知します。集合場所の詳細は承認後に表示されます。</Text></View>}{!isHost&&!hangout.myJoinStatus&&hangout.status==='OPEN'&&<Pressable style={styles.primary} onPress={()=>onJoin(hangout)}><Text style={styles.primaryText}>参加を申請する</Text></Pressable>}{!isHost&&<Pressable style={styles.reportButton} onPress={()=>onReport(hangout)}><Text style={styles.reportText}>この募集の主催者を通報・ブロック</Text></Pressable>}{isHost&&<><Text style={styles.sectionTitle}>参加申請</Text>{requests.map(item=><View key={item.id} style={styles.requestCard}><View style={styles.cardCopy}><Text style={styles.hostName}>{item.user.displayName}{item.user.verification==='PHONE_VERIFIED'?' ・電話確認済み':''}</Text><Text style={styles.muted}>{item.message||'メッセージなし'} ／ {item.status}</Text></View>{item.status==='PENDING'&&<View style={styles.requestActions}><Pressable style={styles.rejectButton} onPress={()=>onDecide(item.id,false)}><Text>見送る</Text></Pressable><Pressable style={styles.acceptButton} onPress={()=>onDecide(item.id,true)}><Text style={styles.primaryText}>承認</Text></Pressable></View>}</View>)}{!requests.length&&<Text style={styles.empty}>まだ申請はありません。</Text>}<Pressable style={styles.finishButtonWide} onPress={()=>onFinish(hangout.id)}><Text style={styles.primaryText}>Hangoutを終了</Text></Pressable></>}</ScrollView>}

function PhoneVerificationScreen({onBack,onVerify}:{onBack:()=>void;onVerify:(phone:string,code?:string)=>Promise<void>}){const [phone,setPhone]=useState('+81');const [code,setCode]=useState('');const [sent,setSent]=useState(false);return <ScrollView contentContainerStyle={styles.formPage}><Pressable onPress={onBack}><Text style={styles.backText}>‹ 戻る</Text></Pressable><Text style={styles.pageTitle}>電話番号を確認</Text><Text style={styles.safetyNote}>安全なコミュニティ運営のため、募集作成にはSMS確認が必要です。番号は他の利用者には公開されません。</Text><Text style={styles.label}>電話番号（国番号付き）</Text><TextInput style={styles.input} value={phone} onChangeText={setPhone} keyboardType="phone-pad" placeholder="+819012345678"/>{sent&&<><Text style={styles.label}>6桁の確認コード</Text><TextInput style={styles.input} value={code} onChangeText={setCode} keyboardType="number-pad" maxLength={6}/></>}<Pressable style={styles.primary} onPress={()=>void onVerify(phone,sent?code:undefined).then(()=>{if(!sent)setSent(true)})}><Text style={styles.primaryText}>{sent?'確認して完了':'SMSを送信'}</Text></Pressable></ScrollView>}

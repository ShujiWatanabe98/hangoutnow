...
@Controller('chat-rooms')@UseGuards(AccessTokenGuard)export class ChatController{constructor(@Inject(ChatService)private readonly s:ChatService){}@Get()rooms(@Req()r:AuthenticatedRequest){return this.s.rooms(r.userId)}@Get(':id/messages')messages(@Req()r:AuthenticatedRequest,@Param('id')id:string){return this.s.messages(r.userId,id)}@Post(':id/messages')send(@Req()r:AuthenticatedRequest,@Param('id')id:string,@Body()b:SendMessageDto){return this.s.send(r.userId,id,b.body,b.stampId)}}
...
@Controller('direct-chats')@UseGuards(AccessTokenGuard)export class DirectChatController{
 ...
 @Post(':id/messages') send(@Req()r:AuthenticatedRequest,@Param('id')id:string,@Body()b:SendMessageDto){return this.s.sendDirect(r.userId,id,b.body,b.stampId)}
}

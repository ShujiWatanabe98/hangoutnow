...
describe('authentication and profile', () => {
  ...
  it('registers an adult and allows authenticated profile updates', async () => {
    ...
    const matching = await request(app.getHttpServer()).patch('/users/me').set('Authorization', `Bearer ${registered.body.accessToken as string}`).send({ preferredAreas: ['新宿', '新宿', '渋谷'], preferredActivities: ['カフェ', 'ランニング'], preferredAgeMin: 25, preferredAgeMax: 40, preferredGenders: ['FEMALE', 'OTHER'], activityTimeSlots: ['夜', '土', '日'], matchingDataConsent: true, participationUrgency: 'TODAY', maxTravelMinutes: 30, preferredGroupSizes: [2, 4], budgetMin: 1000, budgetMax: 5000, socialStyles: ['少人数でじっくり'], participationGoals: ['趣味仲間', '友達づくり'], firstTimePreferences: ['初参加歓迎'], alcoholPreference: 'SOMETIMES', smokingPreference: 'NON_SMOKING', avoidPreferences: ['大人数', '深夜'], scheduleFlexibility: ['途中参加OK', '途中退出OK'], behaviorLearningEnabled: true }).expect(200);
    expect(matching.body).toMatchObject({ preferredAreas: ['新宿', '渋谷'], preferredActivities: ['カフェ', 'ランニング'], preferredAgeMin: 25, preferredAgeMax: 40, preferredGenders: ['FEMALE', 'OTHER'], activityTimeSlots: ['夜', '土', '日'], matchingDataConsent: true, participationUrgency: 'TODAY', maxTravelMinutes: 30, preferredGroupSizes: [2, 4], budgetMin: 1000, budgetMax: 5000, socialStyles: ['少人数でじっくり'], participationGoals: ['趣味仲間', '友達づくり'], firstTimePreferences: ['初参加歓迎'], alcoholPreference: 'SOMETIMES', smokingPreference: 'NON_SMOKING', avoidPreferences: ['大人数', '深夜'], scheduleFlexibility: ['途中参加OK', '途中退出OK'], behaviorLearningEnabled: true });

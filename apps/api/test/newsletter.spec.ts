...
describe('newsletter subscriptions', () => {
  ...
  it('normalizes and stores a new subscriber without exposing a stored token', async () => {
    ...
    const result = await new NewsletterService(db).subscribe({ email: ' Person@Example.COM ', consent: true, source: 'homepage' });
  ...
  it('does not create a duplicate active subscription', async () => {
    ...
    await expect(new NewsletterService(db).subscribe({ email: 'person@example.com', consent: true, source: 'homepage' })).resolves.toEqual({ registered: true, alreadyRegistered: true });
  ...
  it('unsubscribes using only the hashed token lookup', async () => {
    ...
    await expect(new NewsletterService(db).unsubscribe(token)).resolves.toEqual({ unsubscribed: true });

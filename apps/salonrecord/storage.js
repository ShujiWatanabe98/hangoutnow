...
async function health() {
  ...
  const result = await pool.query("SELECT current_schema() AS schema, (current_setting('server_version_num')::integer / 10000) AS version");
  return {
    storage: 'postgres',
    databaseSchema: result.rows[0]?.schema || null,
    databaseServerVersionMajor: Number(result.rows[0]?.version) || null
  };
}

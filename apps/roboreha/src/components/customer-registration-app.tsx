...
function Review({ title, value, edit }: { title: string; value: string; edit: () => void }) { return <div className="rounded-2xl border border-[#dce8e5] p-4"><div className="flex items-start justify-between gap-3"><p className="text-xs font-black text-[#71858a]">{title}</p><button type="button" onClick={edit} className="min-h-9 rounded-lg bg-[#edf5f3] px-3 text-xs font-black text-[#087f71]">修正</button></div><p className="mt-1 whitespace-pre-line text-base font-bold leading-6">{value}</p></div>; }

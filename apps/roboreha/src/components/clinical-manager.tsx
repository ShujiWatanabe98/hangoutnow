...
import { CalendarDays, ChevronRight, CircleStop, Clock3, FileText, Play, Stethoscope, X } from "lucide-react";
...
export function ClinicalManager() {
  ...
  return <div className="mx-auto max-w-[1400px]">
    ...
    <div className="mt-5"><ClinicalSection title="履歴" subtitle={`過去90日・${historyItems.length}件`} icon="history" items={historyItems} empty="施術履歴はありません" onDetail={setDetail} onAction={act} saving={saving} history /></div>
...
function ClinicalSection({ title, dateLabel, subtitle, icon, items, empty, onDetail, onAction, saving, history = false }: { title: string; dateLabel?: string; subtitle: string; icon: "today" | "history"; items: Item[]; empty: string; onDetail: (item: Item) => void; onAction: (id: string, action: "start" | "finish") => void; saving: boolean; history?: boolean }) {
  return <section className="rounded-[24px] border border-[#dce8e5] bg-white p-4"><div className="flex items-center justify-between"><div className="flex flex-wrap items-center gap-2">{icon === "today" ? <CalendarDays size={20} className="text-[#087f71]" /> : <FileText size={20} className="text-[#5769a7]" />}<h3 className="text-lg font-black">{title}</h3>{dateLabel && <span className="rounded-lg bg-[#e7f5f1] px-3 py-1 text-xs font-black text-[#087f71]">{dateLabel}</span>}</div><span className="rounded-full bg-[#edf4f2] px-3 py-1 text-xs font-black text-[#60777c]">{subtitle}</span></div><div className="mt-3 grid gap-2 lg:grid-cols-2">{items.map((item) => <article key={item.id} className="rounded-2xl border border-[#dce8e5] p-3"><div className="flex items-center gap-3"><div className={`grid size-11 shrink-0 place-items-center rounded-xl ${history ? "bg-[#eef1fb] text-[#5769a7]" : "bg-[#e7f5f1] text-[#087f71]"}`}><Stethoscope size={20} /></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="font-black">{item.customer_name}</p><span className="rounded-full bg-[#f0f4f3] px-2 py-0.5 text-[9px] font-black text-[#60777c]">{statusLabel[item.status] ?? item.status}</span></div><p className="truncate text-xs text-[#71858a]">{history ? `${dateText(item.start_at)}・` : `${timeText(item.start_at)}・`}{item.product_name}</p><p className="mt-0.5 truncate text-[10px] text-[#8a9b9f]">{item.therapist_name}・{item.hal_asset_code || "HAL未割当"}</p></div></div><div className="mt-3 flex flex-wrap justify-end gap-2 border-t border-[#e7eeec] pt-3"><button onClick={() => onDetail(item)} className="flex min-h-10 items-center gap-1 rounded-xl bg-[#edf5f3] px-3 text-xs font-black text-[#087f71]">詳細<ChevronRight size={15} /></button>{!history && !item.started_at && <button disabled={item.safety_decision !== "allow" || saving} onClick={() => onAction(item.id, "start")} className="flex min-h-10 items-center gap-1 rounded-xl bg-[#087f71] px-3 text-xs font-black text-white disabled:bg-[#b5c5c2]"><Play size={14} />開始</button>}{!history && item.started_at && !item.ended_at && <button disabled={saving} onClick={() => onAction(item.id, "finish")} className="flex min-h-10 items-center gap-1 rounded-xl bg-[#c35645] px-3 text-xs font-black text-white"><CircleStop size={14} />終了</button>}{item.ended_at && <span className="flex min-h-10 items-center rounded-xl bg-[#e7f5f1] px-3 text-xs font-black text-[#087f71]">終了済み</span>}</div></article>)}{items.length === 0 && <div className="col-span-full rounded-2xl border border-dashed border-[#cfdeda] p-8 text-center text-sm text-[#829397]">{empty}</div>}</div></section>;
}

...
export function ScheduleCalendar() {
  ...
  return <div className="mx-auto max-w-[1500px]">
    <div className="mb-2 flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-black tracking-[0.15em] text-[#087f71]">SCHEDULER</p><h2 className="text-2xl font-black tracking-[-0.04em]">予約スケジュール</h2><p className="mt-1 text-xs text-[#71858a]">クリックで登録、ドラッグで移動、ダブルクリックで詳細表示</p></div><div className="flex w-full items-center rounded-xl border border-[#d7e4e1] bg-white p-1 sm:w-auto"><button onClick={() => setView("therapist")} className={`min-h-11 flex-1 rounded-lg px-3 text-xs font-black sm:flex-none sm:px-4 ${view === "therapist" ? "bg-[#173b42] text-white" : "text-[#71858a]"}`}>療法士</button><button onClick={() => setView("hal")} className={`min-h-11 flex-1 rounded-lg px-3 text-xs font-black sm:flex-none sm:px-4 ${view === "hal" ? "bg-[#173b42] text-white" : "text-[#71858a]"}`}>HAL機器</button><button onClick={() => setView("rehab")} className={`min-h-11 flex-1 rounded-lg px-3 text-xs font-black sm:flex-none sm:px-4 ${view === "rehab" ? "bg-[#173b42] text-white" : "text-[#71858a]"}`}>リハスペース</button></div></div>
    <p className="mb-2 rounded-xl bg-[#eaf5f2] px-3 py-2 text-[11px] font-bold text-[#087f71] sm:hidden">表は左右にスワイプして、療法士・機器・スペースを確認できます。</p>

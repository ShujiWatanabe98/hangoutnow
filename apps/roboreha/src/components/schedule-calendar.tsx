...
export function ScheduleCalendar() {
  ...
  const draggedAppointment = useRef(false);
  ...
  return <div className="mx-auto min-w-0 max-w-[944px]">
    <div className="mb-2 flex min-w-0 flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-black tracking-[0.15em] text-[#087f71]">SCHEDULER</p><h2 className="text-2xl font-black tracking-[-0.04em]">予約スケジュール</h2><p className="mt-1 text-xs text-[#71858a]">空き枠をタップで登録、予定をタップで詳細、長押ししてドラッグで移動</p></div><div className="flex w-full min-w-0 items-center rounded-xl border border-[#d7e4e1] bg-white p-1 sm:w-auto"><button onClick={() => setView("therapist")} className={`min-h-11 flex-1 rounded-lg px-3 text-xs font-black sm:flex-none sm:px-4 ${view === "therapist" ? "bg-[#173b42] text-white" : "text-[#71858a]"}`}>療法士</button><button onClick={() => setView("hal")} className={`min-h-11 flex-1 rounded-lg px-3 text-xs font-black sm:flex-none sm:px-4 ${view === "hal" ? "bg-[#173b42] text-white" : "text-[#71858a]"}`}>HAL機器</button><button onClick={() => setView("rehab")} className={`min-h-11 flex-1 rounded-lg px-3 text-xs font-black sm:flex-none sm:px-4 ${view === "rehab" ? "bg-[#173b42] text-white" : "text-[#71858a]"}`}>リハスペース</button></div></div>
    <p className="mb-2 rounded-xl bg-[#eaf5f2] px-3 py-2 text-[11px] font-bold text-[#087f71] lg:hidden">表は左右にスワイプして、療法士・機器・スペースを確認できます。</p>
    <section className="min-w-0 overflow-hidden rounded-[22px] border border-[#d7e4e1] bg-white shadow-[0_12px_35px_rgba(27,61,58,.06)]">
      <header className="flex items-center justify-center gap-1.5 border-b border-[#dce8e5] px-2 py-3 sm:gap-2 sm:px-4"><button aria-label="前日" onClick={() => shiftDate(-1)} className="grid size-11 shrink-0 place-items-center rounded-lg bg-[#edf4f2] text-[#087f71] hover:bg-[#dff0ec]"><ChevronLeft size={21} /></button><label className="flex min-w-0 flex-1 items-center justify-center gap-1.5 text-sm font-black sm:flex-none sm:gap-2"><CalendarDays size={18} className="hidden shrink-0 text-[#087f71] sm:block" /><input aria-label="表示日" type="date" value={date} onChange={(event) => setDate(event.target.value)} className="min-h-11 min-w-0 w-full rounded-lg border border-[#d7e4e1] px-2 font-black sm:w-auto sm:px-3" /></label><button aria-label="翌日" onClick={() => shiftDate(1)} className="grid size-11 shrink-0 place-items-center rounded-lg bg-[#edf4f2] text-[#087f71] hover:bg-[#dff0ec]"><ChevronRight size={21} /></button><button aria-label="更新" onClick={() => refresh().catch((reason: Error) => setError(reason.message))} className="grid size-11 shrink-0 place-items-center rounded-lg bg-[#edf4f2] text-[#087f71]"><RefreshCw size={17} /></button></header>
      {error && <div role="alert" className="border-b border-[#f0d2cc] bg-[#fff0ed] px-4 py-3 text-sm font-bold text-[#b94637]">{error}</div>}
      {!data ? <div className="p-4"><InlineLoadingState label="予約スケジュールを読み込んでいます" /></div> : <div role="region" aria-label="予約スケジュール表・左右にスクロールできます" tabIndex={0} className="max-h-[calc(100vh-190px)] overflow-auto overscroll-contain focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#087f71]/30">
        <div style={{ minWidth: `${74 + resources.length * 190}px` }}>
          ...
          <div className="grid" style={{ gridTemplateColumns: `74px repeat(${resources.length}, minmax(190px, 1fr))` }}>
            ...
            {resources.map((resource) => { const unavailable = view === "hal" && resource.status !== "available"; return <div key={resource.id} className={`relative border-r border-[#dce8e5] ${unavailable ? "bg-[#f3f5f5]" : ""}`} style={{ height: ROW_HEIGHT * SLOT_COUNT }} onPointerUp={(event) => { if (!pointerDrag || unavailable) return; const distance = Math.hypot(event.clientX - pointerDrag.startX, event.clientY - pointerDrag.startY); if (distance < 8) { draggedAppointment.current = false; setPointerDrag(null); return; } draggedAppointment.current = true; const appointment = data.appointments.find((item) => item.id === pointerDrag.id); if (!appointment) return; const rect = event.currentTarget.getBoundingClientRect(); const index = Math.max(0, Math.min(SLOT_COUNT - 1, Math.floor((event.clientY - rect.top) / ROW_HEIGHT))); void moveAppointment(appointment, resource.id, slots[index]); }}>
              ...
              {data.appointments.filter((appointment) => appointmentResourceId(appointment) === resource.id).map((appointment) => { const start = Math.max(START_MINUTES, minuteOfDay(appointment.start_at)); const end = Math.min(18 * 60, minuteOfDay(appointment.end_at)); const top = ((start - START_MINUTES) / 30) * ROW_HEIGHT; const height = Math.max(44, ((end - start) / 30) * ROW_HEIGHT - 4); const movable = ["reserved", "confirmed"].includes(appointment.status); return <button type="button" key={appointment.id} aria-label={`${timeText(appointment.start_at)} ${appointment.customer_name}の予約詳細`} onPointerDown={(event) => { if (movable && event.button === 0) { draggedAppointment.current = false; setPointerDrag({ id: appointment.id, startX: event.clientX, startY: event.clientY }); } }} onPointerCancel={() => { draggedAppointment.current = false; setPointerDrag(null); }} onClick={() => { if (draggedAppointment.current) { draggedAppointment.current = false; return; } openDetail(appointment); }} title="タップで詳細・長押ししてドラッグで移動" className={`absolute left-1 right-1 z-10 select-none overflow-hidden rounded-lg border-l-4 p-2 text-left shadow-sm ${movable ? "cursor-grab touch-none border-[#087f71] bg-[#dff4ed] hover:ring-2 hover:ring-[#087f71]/30 active:cursor-grabbing" : "cursor-default border-[#6377a9] bg-[#e9edf8]"}`} style={{ top, height }}><span className="flex items-start gap-1"><GripVertical size={13} className="mt-0.5 shrink-0 text-[#66847f]" /><span className="min-w-0"><span className="block truncate text-xs font-black">{timeText(appointment.start_at)} {appointment.customer_name}</span><span className="mt-0.5 block truncate text-[10px] text-[#58716f]">{appointmentSubline(appointment)}</span>{height > 66 && <span className="mt-1 block truncate text-[10px] text-[#71858a]">{appointment.product_name}</span>}</span></span></button>; })}
            </div>; })}
          </div>
        </div>
      </div>}
    ...
    <BlockingProgressOverlay open={saving} label="予約情報を保存しています…" detail="予約枠と機材・療法士の空きを更新しています。完了するまでお待ちください。" />
  </div>;
}

...
export async function POST(request: Request) {
  ...
  if (previous?.lockedUntil && previous.lockedUntil > now) return NextResponse.redirect(new URL(`${ROBOREHA_BASE_PATH}/login?locked=1`, request.url), 303);
  ...
  if (!valid) {
    ...
    return NextResponse.redirect(new URL(`${ROBOREHA_BASE_PATH}/login?error=1`, request.url), 303);
  }

  attempts.delete(key);
  const response = NextResponse.redirect(new URL(`${ROBOREHA_BASE_PATH}/`, request.url), 303);

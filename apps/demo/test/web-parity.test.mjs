...
test('corporate homepage presents the seven methodmore products accurately', async () => {
  const [corporate, hangout, divertnavi, divertnaviPrivacy, sitemap, corporateStyles, divertStyles, coachDemo, coachStyles, coachBootstrap, coachDemoScript, coachDriveModule, coachUnderpassModule, coachPoliceModule, coachNaturalSpeechModule, coachRoadSnappingModule, coachSmoothLocationModule, coachUserReportAggregationModule, coachVoiceApproachModule, coachMonitorPointsJson, coachUnderpassFeedJson, coachPrivacy, coachSupport, coachDataSources, careDemo, careStyles, careApp, carePersonas, careManifestJson, smarihaDemo, smarihaStyles, smarihaScript, smarihaLogin, smarihaLoginScript, smarihaTaisho, smarihaKeijinkai, schedulerDemo, schedulerStyles, schedulerScript, schedulerLogin, schedulerLoginScript, server] = await Promise.all([
    ...
    readFile(new URL('../public/coachgo-demo/styles.css', import.meta.url), 'utf8'),
    ...
    readFile(new URL('../public/smariha-scheduler/index.html', import.meta.url), 'utf8'),
    readFile(new URL('../public/smariha-scheduler/styles.css', import.meta.url), 'utf8'),
    readFile(new URL('../public/smariha-scheduler/app.js', import.meta.url), 'utf8'),
    readFile(new URL('../public/smariha-scheduler/login.html', import.meta.url), 'utf8'),
    readFile(new URL('../public/smariha-scheduler/login.js', import.meta.url), 'utf8'),
    readFile(new URL('../server.mjs', import.meta.url), 'utf8'),
  ]);
  ...
  for (const copy of ['提案用MVP・架空データ', '既存予約・院内予定・単位上限', '療法士別スケジュール', '未配置予約', '安全条件を満たす配置候補', '療法士の承認操作後にだけ反映']) {
    assert.ok(schedulerDemo.includes(copy), `スマリハスケジューラーの画面コピーがありません: ${copy}`);
  }
  assert.match(schedulerStyles, /\.schedule-grid/);
  assert.match(schedulerScript, /createAppointment/);
  assert.match(schedulerScript, /applyFilters/);
  assert.match(schedulerLogin, /action="\/smariha-scheduler\/login"/);
  assert.match(schedulerLogin, /すべて架空のデータ/);
  assert.match(schedulerLoginScript, /ユーザー名またはパスワードが違います/);
  assert.doesNotMatch(sitemap, /<loc>https:\/\/method-more\.com\/smariha-scheduler\/<\/loc>/);
  assert.match(server, /const smarihaSchedulerPath = '\/smariha-scheduler'/);
  assert.match(server, /requestedPath === '\/smariha-scheduler\/'/);
  ...
  assert.match(coachDemo, /id="selected-count" hidden/);
  ...
  assert.match(coachDemo, /href="\/coachgo-demo\/styles\.css\?v=20260907-1"/);
  ...
  assert.doesNotMatch(coachDemo, /地図で確認し、接近したときに通知する危険を選択/);
  assert.match(coachDemo, /id="permission-status" class="permission-status" role="status" hidden/);
  assert.match(coachDemo, /id="input-permission-status" class="permission-status" role="status" hidden/);
  assert.match(coachDemo, /id="hazard-voice-toggle"/);
  assert.match(coachDemo, /<strong>危険地点の読み上げ<\/strong>/);
  assert.match(coachDemo, /id="open-settings"/);
  assert.match(coachDemo, /class="settings-sheet-handle"/);
  assert.match(coachStyles, /\.control-panel\.open/);
  ...
  assert.match(coachDemo, /src="\/coachgo-demo\/bootstrap\.js\?v=20260907-1"/);
  assert.match(coachBootstrap, /\/coachgo-demo\/dist\/mobile\/demo\.js\?v=20260907-1/);
  ...
  assert.match(coachDemoScript, /divertNaviUnderpasses\.js\?v=20260907-1/);
  ...
  assert.doesNotMatch(coachDemoScript, /DivertNavi公開データ: アンダーパス/);
  ...
  assert.match(coachDemoScript, /continuousDemoDrive\.js\?v=20260907-1/);
  assert.match(coachDemoScript, /hazardMap\.js\?v=20260907-1/);
  assert.match(coachDemoScript, /naturalSpeech\.js\?v=20260907-1/);
  assert.match(coachDemoScript, /roadSnapping\.js\?v=20260907-1/);
  assert.match(coachDemoScript, /sharedUserReports\.js\?v=20260907-1/);
  assert.match(coachDemoScript, /smoothUserLocation\.js\?v=20260907-1/);
  assert.match(coachDemoScript, /userReportAggregation\.js\?v=20260907-1/);
  assert.match(coachDemoScript, /voiceApproach\.js\?v=20260907-1/);
  assert.match(coachDemoScript, /voiceInputRuntime\.js\?v=20260907-1/);
  ...
  assert.match(coachDemoScript, /offset: \[0, -36\]/);
  assert.match(coachStyles, /\.user-report-map-marker \{ z-index: 30; \}/);
  assert.match(coachStyles, /\.user-report-map-marker-count/);
  assert.match(coachDemo, /data-registration-method="CURRENT" aria-pressed="true">現在地で登録/);
  assert.match(coachDemo, /data-registration-method="PICKER" aria-pressed="false">位置指定で登録/);
  assert.match(coachDemo, /id="report-location-picker"/);
  assert.match(coachDemo, /id="confirm-report-location"/);
  assert.match(coachDemoScript, /ownedSharedUserReportIds\.has\(id\)/);
  assert.match(coachDemoScript, /loadSharedUserReports\(apiUrl, currentReportOwnerId\)/);
  assert.match(coachDemoScript, /deleteButton\.className = "delete-own-report"/);
  assert.match(coachRoadSnappingModule, /directions\/v5\/mapbox\/driving/);
  assert.match(coachRoadSnappingModule, /MAXIMUM_ROAD_SNAP_DISTANCE_METERS = 100/);
  ...
  assert.match(coachDemoScript, /demoRenderQuality = "BALANCED"/);
  assert.match(coachDemoScript, /\[recenterMapButton, registerHazardButton, demoPlaybackButton\]/);
  assert.match(coachDemoScript, /demoPlaybackButton\.hidden = !demoVisibilityEnabled/);
  assert.match(coachDemoScript, /renderInputSettings\(\);\s+returnToCurrentLocation\(\)/);
  assert.match(coachDemoScript, /function returnToCurrentLocation\(\)/);
  assert.match(coachDemoScript, /showCurrentLocationOnMap\(currentUserLocation, "現在地に戻りました。"\)/);
  assert.match(coachDemoScript, /map\.setLayoutProperty\(layerId, "visibility", routeVisibility\)/);
  assert.match(coachDemoScript, /const DEFAULT_LOCATION_ZOOM = 15\.8/);
  assert.match(coachDemo, /id="map-orientation"/);
  assert.match(coachStyles, /\.compass-control/);
  assert.match(coachDemoScript, /const mapOrientationButton = requiredElement\("#map-orientation"\)/);
  assert.match(coachDemoScript, /animateContinuousUserLocation/);
  assert.match(coachSmoothLocationModule, /function predictUserLocation/);
  assert.doesNotMatch(coachStyles, /transition: transform 100ms linear/);
  assert.match(coachDemoScript, /nearbyMonitoredPointsAtLocation\(location, voiceMonitorPoints\(false\), selectedCategories\)/);
  ...
  assert.match(coachVoiceApproachModule, /function nearbyMonitoredPointsAtLocation/);
  assert.match(coachDemoScript, /navigator\.geolocation\.watchPosition/);
  ...
  assert.match(coachDemoScript, /voiceHazardReport\.js\?v=20260907-1/);
  ...
  assert.match(coachDemoScript, /navigator\.mediaDevices\.getUserMedia\(\{ audio: true \}\)/);
  assert.match(coachDemo, /id="microphone-permission-dialog"/);
  assert.match(coachDemo, /id="grant-microphone-permission"/);
  assert.match(coachDemo, /type="button">続ける<\/button>/);
  assert.match(coachDemo, /id="close-microphone-permission"[^>]*hidden>閉じる<\/button>/);
  assert.doesNotMatch(coachDemo, />マイクを許可する<\/button>/);
  assert.doesNotMatch(coachDemo, />あとで<\/button>/);
  assert.doesNotMatch(coachDemoScript, /ensureMicrophonePermission\("startup"\)/);
  assert.match(coachDemoScript, /ensureMicrophonePermission\("voice-report"\)/);
  assert.match(coachDemoScript, /if \(permissionState === "granted"\)/);
  assert.match(coachDemoScript, /closeMicrophonePermissionButton\.hidden = true/);
  assert.match(coachDemoScript, /if \(closeMicrophonePermissionButton\.hidden\)\s+event\.preventDefault\(\)/);
  assert.match(coachDemoScript, /navigator\.permissions\.query\(\{ name: "microphone" \}/);
  assert.match(coachDemoScript, /function handleMicrophonePermissionFailure/);
  assert.match(coachDemoScript, /if \(voiceInputRuntime\.underDevelopment\) \{\s+voiceInputEnabled = false/);
  assert.match(coachDemoScript, /音声入力はONですが、マイクが許可されていません/);
  assert.match(coachDemoScript, /coachgo:native-location/);
  assert.match(coachDemoScript, /map\?\.jumpTo\(\{\s+center: \[renderedUserLocation\[0\], renderedUserLocation\[1\]\],\s+bearing: desiredMapBearing\(now\),\s+\}\)/);
  assert.match(coachDemoScript, /snapAndRegisterSessionHazard\(match\.category, currentUserLocation, voiceReportStatus\)/);
  ...
  assert.match(coachDemoScript, /DEMO_BALANCED_CAMERA_INTERVAL_MS = 1_000 \/ 30/);
  assert.match(coachDemoScript, /createDemoRouteSampler\(demoDriveRoute\)/);
  assert.match(coachDemoScript, /demoDriveSampler\.positionAt\(demoDriveProgress, DEMO_BEARING_LOOKAHEAD_METERS\)/);
  assert.match(coachDemoScript, /smoothBearing\(demoSmoothedBearing, target\.bearing/);
  ...
  assert.match(coachNaturalSpeechModule, /rate: 1\.16/);
  assert.match(coachNaturalSpeechModule, /isPlaceName \? 1\.1/);
  assert.match(coachNaturalSpeechModule, /pauseAfterMs/);
  assert.match(coachSmoothLocationModule, /STATIONARY_DISTANCE_METERS = 8/);
  assert.match(coachSmoothLocationModule, /function interpolateUserLocation/);
  assert.match(coachSmoothLocationModule, /function userLocationMovementBearing/);
  assert.match(coachDemoScript, /handleUserLocationSample\(location, now, true\)/);
  assert.match(coachDemoScript, /window\.addEventListener\("coachgo:native-heading"/);
  assert.match(coachDemoScript, /zoom: 15/);
  ...
  assert.match(coachDemoScript, /kanagawaPolicePoints\.js\?v=20260907-1/);
  ...
  assert.doesNotMatch(coachDemo, /Weather data by RainViewer/);
  assert.doesNotMatch(coachDemoScript, /RainViewerの最新雨雲レーダーを表示中/);
  assert.match(coachDemoScript, /"raster-opacity": 0\.32/);
...
test('public server sends browser security headers', async () => {
  ...
  assert.match(server, /'permissions-policy': 'camera=\(self\), geolocation=\(self\), microphone=\(self\)'/);

...
test('corporate homepage presents the seven methodmore products accurately', async () => {
  ...
  assert.match(coachDemoScript, /\[recenterMapButton, mapOrientationButton, registerHazardButton, demoPlaybackButton\]/);
  assert.match(coachDemo, /現在地・方角・投稿・デモのボタンを大きくします/);
  assert.match(coachStyles, /\.compass-control\.large-input-icon \.compass-control-content strong \{ font-size: 13px; \}/);
  assert.match(coachStyles, /\.compass-control\.large-input-icon \.compass-control-content svg \{ width: 34px; height: 34px;/);

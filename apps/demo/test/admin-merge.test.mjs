...
test('Hangout Now admin is exported and served by the demo service', async () => {
  const [renderConfig, nextConfig, server, adminPage] = await Promise.all([
  ...
  assert.match(renderConfig, /name: hangoutnow-demo[\s\S]*buildCommand: npm ci && npm run build -w @hangout-now\/admin/);

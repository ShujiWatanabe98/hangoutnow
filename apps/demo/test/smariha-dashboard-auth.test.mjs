...
test('canonical rehainfo source UI is login-protected and serves every audited source template', async () => {
  ...
  assert.deepEqual(emrRelease, {
    ...
    version: '0.5.0',
    release: '2026-09-12-patient-lookup-performer-type-fix',
  ...
  for (const [path, source, title, marker] of pages) {
    ...
    assert.match(html, /\/rehainfo\/source-demo-adapter\.js\?v=20260913-1/);
  ...
  const therapistCsv = await fetch(`${origin}/rehainfo/schedule-management/api/export.csv?month=2026-09&type=therapist`, { headers: { cookie } });
  assert.equal(therapistCsv.status, 200);
  assert.match(therapistCsv.headers.get('content-type'), /text\/csv/);
  assert.match(therapistCsv.headers.get('content-disposition'), /attachment/);
  assert.match(await therapistCsv.text(), /PT01/);
  ...
  assert.match(prescriptionListHtml, /id="prescriptionPatientLookup"[^>]*>患者IDから取得<\/button>/);

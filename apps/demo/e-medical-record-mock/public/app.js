...
async function renderEncounter() {
  ...
  <aside class="stack"><article class="card"><div class="card-header"><h3>患者サマリー</h3></div><div class="card-body summary-list"><div class="summary-item"><small>アレルギー</small><p>${patient.allergies.length ? patient.allergies.map((a) => `<span class="tag tag-danger">${escapeHtml(a)}</span>`).join('') : 'なし'}</p></div><div class="summary-item"><small>現病・プロブレム</small><p>高血圧症（架空データ）</p></div><div class="summary-item"><small>継続処方</small><p>${summary.medications.map((m) => escapeHtml(m.medicationDisplay)).join('<br>') || 'なし'}</p></div><div class="summary-item"><small>直近検査</small><p>${summary.labs.map((l) => `${escapeHtml(l.name)}：${escapeHtml(l.result || '結果待ち')}`).join('<br>') || 'なし'}</p></div></div></article><div class="notice-box warning-box"><strong>確定前確認</strong><p>確定後の直接編集は不可とし、訂正履歴を追加する設計です。モックでは新規作成と確定を再現します。</p></div></aside></section>`;
...
function drawEncounterTab(tab) {
  ...
  if (tab === 'soap') panel.innerHTML = `<div class="soap-grid"><label class="soap-field"><b><span>S</span>Subjective / 主観</b><textarea id="soap-s">朝から咳と痰があります。発熱はありません。</textarea></label><label class="soap-field"><b><span>O</span>Objective / 客観</b><textarea id="soap-o">体温36.7℃、SpO2 98%、呼吸音に明らかな異常なし。</textarea></label><label class="soap-field"><b><span>A</span>Assessment / 評価</b><textarea id="soap-a">急性上気道炎を疑う。</textarea></label><label class="soap-field"><b><span>P</span>Plan / 計画</b><textarea id="soap-p">対症療法。悪化時または1週間以上続く場合は再診。</textarea></label></div>`;

...
async function chatScreen() {
  ...
  const groupRows=groups.map((r,i)=>`<div class="group-chat-card"><button class="chat-row" data-room="${r.id}" data-kind="group"><span class="mini ${portraitClass(i%4)}"></span><span><b>${r.hangout.title}</b><small>${r.members.length}人 ・ ${r.lastMessage?.body||'グループにメッセージを送りましょう'}</small></span><em>グループ</em></button><div class="member-actions">${r.members.filter(member=>member.id!==session.user.id).map(member=>`<button data-start-direct="${member.id}">${member.displayName}と1対1</button>`).join('')}</div></div>`).join('');
  const directRows=directs.map((r,i)=>`<button class="chat-row" data-room="${r.id}" data-kind="direct"><span class="mini ${portraitClass((i+1)%4)}"${photoStyle(r.otherUser.profilePhoto)}></span><span><b>${r.otherUser.displayName}</b><small>${r.lastMessage?.body||'1対1でメッセージを送りましょう'}</small></span><em>1対1</em></button>`).join('');
  shell(`<section class="page-title chat-page-title"><div><div class="eyebrow">メッセージをひとつの画面で</div><h1>チャット</h1></div><span class="realtime-pill">● リアルタイム</span></section><div class="chat-tabs"><button class="active" data-chat-tab="group">グループ <b>${groups.length}</b></button><button data-chat-tab="direct">1対1 <b>${directs.length}</b></button></div><section class="chat-list" data-chat-list="group">${groupRows||'<div class="empty">参加が承認されるとグループチャットが表示されます。</div>'}</section><section class="chat-list hidden" data-chat-list="direct">${directRows||'<div class="empty">グループの参加者から相手を選ぶと1対1チャットを開始できます。</div>'}</section>`, false);
  ...
  document.querySelectorAll('[data-room]').forEach((button)=>button.onclick=()=>openChat(button.dataset.room,button.dataset.kind));
  document.querySelectorAll('[data-start-direct]').forEach((button)=>button.onclick=async()=>{try{const room=await api('/direct-chats',{method:'POST',body:JSON.stringify({userId:button.dataset.startDirect})});openChat(room.id,'direct')}catch(error){toast(error.message)}});
}
...
async function openChat(id,kind='group') {
  const base=kind==='direct'?'/direct-chats':'/chat-rooms';
  let messages=[];try{messages=await api(`${base}/${id}/messages`)}catch(error){toast(error.message);return}
  document.body.insertAdjacentHTML('beforeend',`<div class="sheet"><section class="panel chat-panel"><div class="chat-conversation-head"><button class="chat-back" type="button" aria-label="チャット一覧に戻る">＜</button><div><div class="conversation-label">${kind==='direct'?'1対1チャット':'グループチャット'}</div><h2>${kind==='direct'?'プライベートな会話':'Hangout参加者の会話'}</h2></div></div><div class="messages">${messages.map((m)=>`<div class="bubble ${m.senderUserId===session.user.id?'mine':''}"><small>${m.sender.displayName}</small>${m.body}</div>`).join('')||'<div class="empty">最初のメッセージを送ってみましょう。</div>'}</div>${kind==='group'?'<div class="quick"><button>向かっています</button><button>少し遅れます</button><button>到着しました</button></div>':''}<div class="composer"><input placeholder="メッセージ" maxlength="1000"><button>送信</button></div></section></div>`);
  const sheet=document.querySelector('.sheet'); sheet.querySelector('.chat-back').onclick=()=>sheet.remove();
  const send=async(text)=>{if(!text.trim())return;await api(`${base}/${id}/messages`,{method:'POST',body:JSON.stringify({body:text.trim()})});sheet.remove();openChat(id,kind)};

...
function phoneAuthDialog(){document.body.insertAdjacentHTML('beforeend',`<div class="sheet phone-auth-sheet"><section class="panel"><div class="handle"></div><h2>電話番号で続ける</h2><p>SMSで届く6桁の認証コードを入力してください。</p><label>電話番号（国番号から）</label><input id="auth-phone" type="tel" inputmode="tel" value="+81"><div id="auth-phone-code-area" class="hidden"><label>6桁の認証コード</label><input id="auth-phone-code" inputmode="numeric" maxlength="6" autocomplete="one-time-code"><small id="auth-phone-help"></small></div><button class="primary" id="auth-phone-action">認証コードを送る</button><button class="secondary" id="auth-phone-close">キャンセル</button></section></div>`);const sheet=document.querySelector('.phone-auth-sheet');const action=sheet.querySelector('#auth-phone-action');let challengeToken=null;sheet.querySelector('#auth-phone-close').onclick=()=>sheet.remove();action.onclick=async()=>{action.disabled=true;try{const phone=sheet.querySelector('#auth-phone').value.trim();if(!challengeToken){const result=await api('/auth/phone/request',{method:'POST',body:JSON.stringify({phone})});challengeToken=result.challengeToken;sheet.querySelector('#auth-phone-code-area').classList.remove('hidden');sheet.querySelector('#auth-phone-help').textContent=result.demoCode?`開発用コード：${result.demoCode}`:'SMSに認証コードを送信しました';action.textContent='アカウント作成・ログイン';return}const result=await api('/auth/phone/confirm',{method:'POST',body:JSON.stringify({phone,challengeToken,code:sheet.querySelector('#auth-phone-code').value.trim()})});session=result;demoRole=null;localStorage.removeItem(DEMO_ROLE_STORAGE_KEY);saveSession();connectRealtime();sheet.remove();await Promise.all([loadNotificationCount(),loadHangouts()]);navigate('home')}catch(error){sheet.querySelector('#auth-phone-help').textContent=error.message}finally{action.disabled=false}}}
...
async function detail(id, sourceScreen = null, options = {}) {
  ...
  if(h.status==='FINISHED'&&(mine||h.myJoinStatus==='ACCEPTED')){try{const rooms=await api('/chat-rooms');detailRatingMembers=(rooms.find(room=>room.hangout.id===id)?.members||[]).filter(member=>member.id!==session.user.id)}catch(error){toast(error.message)}}
...
function showProfileEditor(){
  ...
  genderSelect.insertAdjacentHTML('afterend',`<div class="matching-preferences"><h3>マッチング設定</h3><p class="matching-privacy">入力は任意です。位置は市区・駅などのおおまかなエリアだけを保存し、正確なGPS位置は保存しません。</p><label>希望エリア（読点区切り）</label><input id="edit-preferred-areas" maxlength="300" placeholder="例：新宿、渋谷" value="${safeText((session.user.preferredAreas||[]).join('、'))}"><label>希望する活動（読点区切り）</label><input id="edit-preferred-activities" maxlength="500" placeholder="例：カフェ、ランニング" value="${safeText((session.user.preferredActivities||[]).join('、'))}"><div class="matching-age-row"><label>希望年齢 下限<input id="edit-preferred-age-min" type="number" min="18" max="100" value="${session.user.preferredAgeMin??''}"></label><label>上限<input id="edit-preferred-age-max" type="number" min="18" max="100" value="${session.user.preferredAgeMax??''}"></label></div><label>希望する相手</label><div class="interest-picker matching-gender-picker">${[['MALE','男性'],['FEMALE','女性'],['OTHER','その他'],['UNDISCLOSED','指定なし']].map(([value,label])=>`<button type="button" data-preferred-gender="${value}" class="${(session.user.preferredGenders||[]).includes(value)?'chosen':''}">${label}</button>`).join('')}</div><label>活動しやすい時間（読点区切り）</label><input id="edit-activity-time-slots" maxlength="200" placeholder="例：平日夜、土日昼" value="${safeText((session.user.activityTimeSlots||[]).join('、'))}"><label>参加したい時期</label><select id="edit-participation-urgency"><option value="">未設定</option><option value="NOW">今すぐ</option><option value="TODAY">今日</option><option value="THIS_WEEK">今週</option><option value="WEEKEND">週末</option><option value="FLEXIBLE">いつでも</option></select><label>移動できる時間（分）</label><input id="edit-max-travel-minutes" type="number" min="5" max="180" step="5" placeholder="例：30" value="${session.user.maxTravelMinutes??''}"><label>希望人数（読点区切り）</label><input id="edit-preferred-group-sizes" maxlength="40" placeholder="例：2、4、6" value="${safeText((session.user.preferredGroupSizes||[]).join('、'))}"><div class="matching-age-row"><label>予算 下限（円）<input id="edit-budget-min" type="number" min="0" max="100000" step="500" value="${session.user.budgetMin??''}"></label><label>上限（円）<input id="edit-budget-max" type="number" min="0" max="100000" step="500" value="${session.user.budgetMax??''}"></label></div><label class="matching-consent"><input id="edit-matching-consent" type="checkbox" ${session.user.matchingDataConsent?'checked':''}> この情報とアプリ内の閲覧・参加履歴を、マッチング改善に利用することに同意します</label></div>`);
  sheet.querySelectorAll('[data-preferred-gender]').forEach(button=>button.onclick=()=>button.classList.toggle('chosen'));
  sheet.querySelector('#edit-participation-urgency').value=session.user.participationUrgency||'';
  ...
  const saveProfileChanges=async()=>{const displayName=sheet.querySelector('#edit-display-name').value.trim();if(!displayName){toast('表示名を入力してください');return false}const parseList=value=>[...new Set(value.split(/[、,]/).map(item=>item.trim()).filter(Boolean))];const parsed=parseList(sheet.querySelector('#edit-interests').value).filter(value=>!INTEREST_OPTIONS.includes(value));const selectedPreset=[...sheet.querySelectorAll('[data-profile-interest].chosen')].map(button=>button.dataset.profileInterest);const updatedInterests=[...new Set([...selectedPreset,...parsed])].slice(0,20);const ageMin=sheet.querySelector('#edit-preferred-age-min').value;const ageMax=sheet.querySelector('#edit-preferred-age-max').value;const budgetMin=sheet.querySelector('#edit-budget-min').value;const budgetMax=sheet.querySelector('#edit-budget-max').value;if(ageMin&&ageMax&&Number(ageMin)>Number(ageMax)){toast('希望年齢の下限は上限以下にしてください');return false}if(budgetMin&&budgetMax&&Number(budgetMin)>Number(budgetMax)){toast('予算の下限は上限以下にしてください');return false}try{session.user=await api('/users/me',{method:'PATCH',body:JSON.stringify({displayName,homeArea:sheet.querySelector('#edit-home-area').value.trim()||null,bio:sheet.querySelector('#edit-bio').value.trim()||null,interests:updatedInterests,gender:sheet.querySelector('#edit-gender').value,preferredAreas:parseList(sheet.querySelector('#edit-preferred-areas').value).slice(0,10),preferredActivities:parseList(sheet.querySelector('#edit-preferred-activities').value).slice(0,20),preferredAgeMin:ageMin?Number(ageMin):null,preferredAgeMax:ageMax?Number(ageMax):null,preferredGenders:[...sheet.querySelectorAll('[data-preferred-gender].chosen')].map(button=>button.dataset.preferredGender),activityTimeSlots:parseList(sheet.querySelector('#edit-activity-time-slots').value).slice(0,7),participationUrgency:sheet.querySelector('#edit-participation-urgency').value||null,maxTravelMinutes:sheet.querySelector('#edit-max-travel-minutes').value?Number(sheet.querySelector('#edit-max-travel-minutes').value):null,preferredGroupSizes:parseList(sheet.querySelector('#edit-preferred-group-sizes').value).map(Number).filter(value=>Number.isInteger(value)&&value>=2&&value<=20).slice(0,6),budgetMin:budgetMin?Number(budgetMin):null,budgetMax:budgetMax?Number(budgetMax):null,matchingDataConsent:sheet.querySelector('#edit-matching-consent').checked,...(profilePhotosChanged?{profilePhotos:pendingProfilePhotos.filter(Boolean).slice(0,3)}:{})})});saveSession();await profileScreen({animate:false});closeProfileEditor();toast('プロフィールとマッチング設定を更新しました');return true}catch(error){toast(error.message);return false}};

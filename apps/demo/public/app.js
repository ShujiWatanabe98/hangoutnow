...
function phoneDialog(){document.body.insertAdjacentHTML('beforeend',`<div class="sheet profile-phone-sheet"><section class="panel"><div class="handle"></div><h2>電話番号確認</h2><p>SMSで届く6桁の認証コードを入力してください。電話番号は他の利用者には公開されません。</p><label>携帯電話番号</label><input id="phone" type="tel" inputmode="tel" autocomplete="tel" placeholder="09012345678" value="${session.user.phoneNumber||''}"><small>090・080・070から、そのまま入力できます。</small><div id="phone-code-area" class="hidden"><label>6桁の認証コード</label><input id="phone-code" inputmode="numeric" maxlength="6" autocomplete="one-time-code"><small id="demo-code"></small></div><button class="primary" id="phone-action">SMSを送信</button><button class="secondary" id="close">キャンセル</button></section></div>`);const sheet=document.querySelector('.profile-phone-sheet');const action=sheet.querySelector('#phone-action');sheet.querySelector('#close').onclick=()=>sheet.remove();let requestedPhone=null;action.onclick=async()=>{action.disabled=true;try{const phone=normalizeJapanesePhone(sheet.querySelector('#phone').value.trim());if(!/^\+[1-9]\d{7,14}$/.test(phone))throw new Error('携帯電話番号を正しく入力してください');if(!requestedPhone){const result=await api('/users/me/phone/request',{method:'POST',body:JSON.stringify({phone})});requestedPhone=phone;sheet.querySelector('#phone').value=phone;sheet.querySelector('#phone').disabled=true;sheet.querySelector('#phone-code-area').classList.remove('hidden');sheet.querySelector('#demo-code').textContent=result.demoCode?`デモ確認コード：${result.demoCode}`:'SMSに認証コードを送信しました';action.textContent='確認して完了';return}session.user=await api('/users/me/phone/confirm',{method:'POST',body:JSON.stringify({phone:requestedPhone,code:sheet.querySelector('#phone-code').value.trim()})});saveSession();sheet.remove();profileScreen();toast('電話番号を確認しました')}catch(error){toast(error.message)}finally{action.disabled=false}}}

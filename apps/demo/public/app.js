...
function shell(content, showFab = true) {
  ...
  const demoGuide=demoRole==='host'?'作成・承認・終了・★5評価を操作':'参加申請・チャット・★5評価を操作';
...
function home() {
  ...
  const journey=demoRole?`<section class="demo-journey"><b>デモ：Hangoutの一連の流れ</b><ol><li>主催者が「＋」で募集を作成</li><li>役割を切り替え、参加者が申請</li><li>主催者が承認し、グループで会話</li><li>主催者が終了して双方が★5評価</li><li>1対1チャットを開始</li></ol><small>「【デモ手順】新宿カフェ交流会」でも途中から試せます。</small></section>`:'';
...
function card(h) {
  ...
  return `<article class="card ${requested ? 'requested' : ''}" data-id="${h.id}"><div class="card-top"><div class="icon">${h.icon}</div><div><h3>${h.title}</h3><div class="meta"><span class="${h.hot ? 'hot' : ''}">${h.time}</span> ・ ${h.place}${h.distanceKm!==null&&h.distanceKm!==undefined?` ・ ${h.distanceKm}km`:''}</div><div class="meta people" style="margin-top:5px">👥 ${h.current} / ${h.max}人 ・ ${conditions} ${requested ? '<b>・申請済み</b>' : ''}${h.distanceKm>10?'<b class="far">・遠め</b>':''}</div></div></div><div class="card-bottom"><div class="host"><span class="mini ${portraitClass(h.photo)}"${photoStyle(h.hostPhoto)} aria-label="${h.host}のプロフィール写真"></span><span>${h.host} ${h.verified?'✓':''}<br>${h.rating}</span></div><span class="match">相性 ${h.match}%</span></div></article>`;
}
...
async function chatScreen() {
  ...
  const demoChatHint=demoRole?`<div class="demo-chat-hint"><span>${demoRole==='host'?'会話後「Hangoutを終了」→参加者を★5評価':'主催者が終了後、主催者を★5評価→1対1を開始'}</span><button id="chat-switch-demo-role">役割切替</button></div>`:'';
  ...
  document.querySelectorAll('[data-rate-five]').forEach((button)=>button.onclick=async()=>{try{await api(`/hangouts/${button.dataset.hangoutId}/ratings`,{method:'POST',body:JSON.stringify({ratedUserId:button.dataset.rateFive,score:5})});toast('★5を送りました。相手も★5を付けると1対1チャットが解放されます');chatScreen()}catch(error){toast(error.message)}});
}
...
async function openChat(room) {
  ...
  conversation.innerHTML=`<header class="chat-conversation-head"><button class="chat-back" type="button" aria-label="チャット一覧に戻る">＜</button><span class="conversation-avatar"${photoStyle(photo)}>${photo?'':safeText(title).slice(0,1)}</span><div><h2>${safeText(title)}</h2><small>${kind==='direct'?'1対1':'グループ ・ '+room.members.length+'人'}　<span>● オンライン</span></small></div></header><div class="messages">${messages.map((m)=>{const mine=m.senderUserId===session.user.id;return `<div class="message-line ${mine?'mine':''}">${mine?'':`<span class="message-avatar"${photoStyle(m.sender.profilePhoto)}>${m.sender.profilePhoto?'':safeText(m.sender.displayName).slice(0,1)}</span>`}<div class="bubble-wrap"><small>${safeText(m.sender.displayName)}</small><div class="bubble">${messageContent(m.body)}</div><time>${new Date(m.createdAt).toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'})}</time></div>${mine?`<span class="message-avatar"${photoStyle(m.sender.profilePhoto||session.user.profilePhoto)}>${m.sender.profilePhoto||session.user.profilePhoto?'':safeText(m.sender.displayName).slice(0,1)}</span>`:''}</div>`}).join('')||'<div class="empty">最初のメッセージを送ってみましょう。</div>'}</div><div class="conversation-actions"><div class="stamp-tray">${stamps.map(stamp=>`<button data-stamp="${stamp.id}" class="stamp-choice"><img src="${safeText(stamp.imageUrl)}" alt="${safeText(stamp.text)}"><b>${safeText(stamp.text)}</b></button>`).join('')}<button class="stamp-create">＋<small>スタンプ作成</small></button></div>${kind==='group'?'<div class="quick"><button>向かっています</button><button>少し遅れます</button><button>到着しました</button></div>':''}<div class="composer"><input placeholder="メッセージを入力" maxlength="1000"><button>送信</button></div></div>`;

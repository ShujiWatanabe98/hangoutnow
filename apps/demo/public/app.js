...
async function detail(id, sourceScreen = null, options = {}) {
  ...
  if(mine&&h.status==='FINISHED'){sheet.querySelector('#delete-hangout').onclick=async()=>{if(!confirm('このHangoutを削除しますか？Hangoutのトークもすべて削除されます。'))return;try{await api(`/hangouts/${id}`,{method:'DELETE'});sheet.remove();await loadHangouts();home();toast('Hangoutとトークを削除しました')}catch(error){toast(error.message)}};return}

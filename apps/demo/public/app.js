...
async function detail(id, sourceScreen = null, options = {}) {
  ...
  const returnAfterDeletion=async()=>{await loadHangouts();if(returnToProfile)await profileScreen();else home()};
  ...
  if(!mine&&h.status==='FINISHED'&&h.myJoinStatus==='ACCEPTED'){try{const rooms=await api('/chat-rooms');detailRatingMembers=(rooms.find(room=>room.hangout.id===id)?.members||[]).filter(member=>member.id!==session.user.id)}catch(error){toast(error.message)}}
  ...
  const detailRatingSection=detailRatingMembers.length?`<section class="detail-rating-section"><div class="eyebrow">HANGOUT終了後</div><h3>主催者・参加者を評価</h3><p>一緒に過ごしたメンバーを★1〜5で評価できます。</p><div class="hangout-rating-members">${detailRatingMembers.map(member=>`<article class="hangout-rating-member"><b>${safeText(member.displayName)}</b><small>${member.id===h.hostUserId?'主催者として評価':'参加者として評価'}</small><div class="rating-stars compact">${[1,2,3,4,5].map(score=>`<button type="button" data-detail-rating="${member.id}" data-score="${score}" class="${member.myRatingScore===score?'selected':''}" aria-label="${safeText(member.displayName)}を星${score}で評価">${score}★</button>`).join('')}</div><em>${member.myRatingScore?`評価済み ★${member.myRatingScore}`:'評価を選択してください'}</em></article>`).join('')}</div></section>`:'';
  ...
  const ownerButtonLabels={
    ...
    '#cancel-hangout':'Hangout削除',
    '#delete-hangout':'Hangout削除',
    '#start-hangout':'Hangout開始',
    '#finish-hangout':'Hangout終了',
  };
  ...
  const deleteButton=sheet.querySelector('#cancel-hangout');
  if(deleteButton)deleteButton.addEventListener('click',async event=>{event.preventDefault();event.stopImmediatePropagation();if(!confirm('このHangoutを削除しますか？Hangoutのトークもすべて削除されます。'))return;try{await api(`/hangouts/${id}`,{method:'DELETE'});sheet.remove();await returnAfterDeletion();toast('Hangoutとトークを削除しました')}catch(error){toast(error.message)}},{capture:true});
...
function showJoinRequestDialog(h,detailSheet){document.querySelector('.join-request-screen')?.remove();document.body.insertAdjacentHTML('beforeend',`<div class="profile-screen join-request-screen"><header class="host-menu-header"><button class="brand-back" type="button" aria-label="Hangoutに戻る"><span></span></button><div><small>参加申請</small><b>参加したい</b></div><span></span></header><main class="profile-screen-content"><section class="host-menu-form join-request-form"><div class="eyebrow">主催者が参加可否を判断します</div><h2>ひとこと添えて申請</h2><p>参加したい理由や当日の雰囲気が伝わるメッセージを書いてください。</p><label>主催者へのメッセージ</label><textarea id="join-message" rows="5" maxlength="200" placeholder="例：カフェ巡りが好きです。初参加ですが、よろしくお願いします！"></textarea><small><span id="join-message-count">0</span> / 200文字</small><button class="primary" id="send-join-request" disabled>この内容で参加申請する</button></section></main></div>`);const dialog=document.querySelector('.join-request-screen');const closeDialog=()=>{dialog.classList.remove('open');dialog.classList.add('closing');setTimeout(()=>dialog.remove(),240)};setTimeout(()=>dialog.classList.add('open'),0);dialog.querySelector('.brand-back').onclick=closeDialog;const input=dialog.querySelector('#join-message');const send=dialog.querySelector('#send-join-request');input.oninput=()=>{const length=input.value.trim().length;dialog.querySelector('#join-message-count').textContent=String(length);send.disabled=length<1};send.onclick=async()=>{const message=input.value.trim();if(!message||send.disabled)return;send.disabled=true;send.textContent='申請中…';try{const request=await api(`/hangouts/${h.id}/join`,{method:'POST',body:JSON.stringify({message})});h.myJoinStatus=request.status;joined.add(h.id);persist();await loadHangouts();dialog.remove();detailSheet.remove();home();toast(request.status==='WAITLISTED'?'待機リストに登録しました':'ひとこと付きで参加申請を送りました')}catch(error){send.disabled=false;send.textContent='この内容で参加申請する';toast(error.message)}}}

function showJoinRequestDialogReliable(h,detailSheet){
...
showJoinRequestDialog=showJoinRequestDialogReliable;
...
async function showHangoutRatingScreenStable(hangoutId){
  try{
    const groups=await api('/chat-rooms');
    const room=groups.find(item=>item.hangout.id===hangoutId);
    const members=(room?.members||[]).filter(member=>member.id!==session.user.id);
    document.body.insertAdjacentHTML('beforeend',`<div class="sheet rating-sheet hangout-rating-sheet"><section class="panel"><div class="handle"></div><div class="eyebrow">Hangoutを終了しました</div><h2>参加メンバーを評価</h2><p>一緒に過ごしたメンバーを★1〜5で評価してください。評価は後から変更できます。</p><div class="hangout-rating-members">${members.map(member=>`<article class="hangout-rating-member"><b>${safeText(member.displayName)}</b><small>${member.id===room.hangout.hostUserId?'主催者として評価':'参加者として評価'}</small><div class="rating-stars compact">${[1,2,3,4,5].map(score=>`<button data-member-rating="${member.id}" data-score="${score}" class="${member.myRatingScore===score?'selected':''}" aria-label="${safeText(member.displayName)}を星${score}で評価">${score}★</button>`).join('')}</div><em>${member.myRatingScore?`評価済み ★${member.myRatingScore}`:'評価を選択してください'}</em></article>`).join('')||'<div class="empty">評価する参加メンバーはいません。</div>'}</div><button class="primary" data-finish-ratings>評価を完了</button></section></div>`);
    const sheet=[...document.querySelectorAll('.hangout-rating-sheet')].at(-1);
    sheet.querySelector('[data-finish-ratings]').onclick=()=>{sheet.remove();home()};
...
showHangoutRatingScreen=showHangoutRatingScreenStable;
...
function showProfileEditor(){
  ...
  document.body.insertAdjacentHTML('beforeend',`<div class="sheet profile-editor-sheet"><section class="panel profile-editor"><div class="handle"></div><h2>プロフィールを編集</h2><label>プロフィール画像（最大3枚）</label>${profilePhotoTrio(session.user,session.user.displayName)}<label class="upload-button profile-editor-photo">画像を選び直す<input id="edit-profile-photo" type="file" accept="image/jpeg,image/png,image/webp" multiple></label><small>中央にメイン画像、2枚目・3枚目は左右の小さい丸に表示します。</small><label>表示名</label><input id="edit-display-name" maxlength="40" value="${safeText(session.user.displayName)}"><label>電話番号</label><button class="outline profile-editor-phone" id="edit-phone">${session.user.verificationStatus==='PHONE_VERIFIED'?'電話番号を変更':'電話番号を確認'}</button><label>活動エリア</label><input id="edit-home-area" maxlength="80" placeholder="例：新宿・渋谷" value="${safeText(session.user.homeArea||'')}"><label>自己紹介</label><textarea id="edit-bio" maxlength="500" rows="5" placeholder="好きなことや参加したいHangoutを書きましょう">${safeText(session.user.bio||'')}</textarea><label>興味のあること</label><input id="edit-interests" maxlength="300" placeholder="カフェ、ランニング、ラーメン" value="${safeText(interests)}"><small>「、」またはカンマで区切って20個まで登録できます。</small><label>性別</label><select id="edit-gender"><option value="UNDISCLOSED">回答しない</option><option value="MALE">男性</option><option value="FEMALE">女性</option><option value="OTHER">その他</option></select><button class="primary" id="save-profile">変更を保存</button><button class="secondary" id="cancel-profile-edit">キャンセル</button></section></div>`);
  const sheet=document.querySelector('.profile-editor-sheet');const closeProfileEditor=()=>{sheet.classList.remove('open');sheet.classList.add('closing');setTimeout(()=>sheet.remove(),240)};setTimeout(()=>sheet.classList.add('open'),0);sheet.querySelector('#edit-gender').value=session.user.gender||'UNDISCLOSED';sheet.querySelector('#cancel-profile-edit').onclick=closeProfileEditor;sheet.querySelector('#edit-phone').onclick=()=>{closeProfileEditor();phoneDialog()};sheet.querySelector('#edit-profile-photo').onchange=event=>{if(event.target.files.length>3){toast('プロフィール画像は3枚まで選択できます');event.target.value=''}};sheet.querySelector('#save-profile').onclick=async()=>{const displayName=sheet.querySelector('#edit-display-name').value.trim();if(!displayName){toast('表示名を入力してください');return false}const parsed=sheet.querySelector('#edit-interests').value.split(/[、,]/).map(value=>value.trim()).filter(Boolean);const interests=[...new Set(parsed)].slice(0,20);try{const files=[...sheet.querySelector('#edit-profile-photo').files];const profilePhotos=files.length?await Promise.all(files.map(imageData)):undefined;session.user=await api('/users/me',{method:'PATCH',body:JSON.stringify({displayName,homeArea:sheet.querySelector('#edit-home-area').value.trim()||null,bio:sheet.querySelector('#edit-bio').value.trim()||null,interests,gender:sheet.querySelector('#edit-gender').value,...(profilePhotos?{profilePhotos}:{})})});saveSession();await profileScreen({animate:false});closeProfileEditor();toast('プロフィールを更新しました');return true}catch(error){toast(error.message);return false}};
  ...
  profilePhotoInput.onchange=async event=>{const file=event.target.files[0];if(!file)return;try{const photo=await imageData(file);pendingProfilePhotos[selectedProfilePhotoIndex]=photo;profilePhotosChanged=true;const button=sheet.querySelector(`[data-profile-photo-index="${selectedProfilePhotoIndex}"]`);button.style.backgroundImage=`url('${photo}')`;button.textContent='';button.classList.remove('empty');toast('新しい画像を表示しました')}catch(error){toast(error.message)}finally{event.target.value=''}};
  sheet.querySelector('#save-profile').onclick=async()=>{const displayName=sheet.querySelector('#edit-display-name').value.trim();if(!displayName){toast('表示名を入力してください');return false}const parsed=sheet.querySelector('#edit-interests').value.split(/[、,]/).map(value=>value.trim()).filter(Boolean).filter(value=>!INTEREST_OPTIONS.includes(value));const selectedPreset=[...sheet.querySelectorAll('[data-profile-interest].chosen')].map(button=>button.dataset.profileInterest);const updatedInterests=[...new Set([...selectedPreset,...parsed])].slice(0,20);try{session.user=await api('/users/me',{method:'PATCH',body:JSON.stringify({displayName,homeArea:sheet.querySelector('#edit-home-area').value.trim()||null,bio:sheet.querySelector('#edit-bio').value.trim()||null,interests:updatedInterests,gender:sheet.querySelector('#edit-gender').value,...(profilePhotosChanged?{profilePhotos:pendingProfilePhotos.filter(Boolean).slice(0,3)}:{})})});saveSession();await profileScreen({animate:false});closeProfileEditor();toast('プロフィールを更新しました');return true}catch(error){toast(error.message);return false}};
  const profileSaveButton=sheet.querySelector('#save-profile');
  const saveProfileChanges=profileSaveButton.onclick;
  profileSaveButton.remove();
  sheet.querySelector('#cancel-profile-edit')?.remove();
  sheet.querySelector('.handle')?.remove();
  sheet.insertAdjacentHTML('afterbegin','<header class="host-menu-header profile-editor-header"><button class="brand-back" type="button" aria-label="プロフィールに戻る"><span></span></button><div><small>アカウント</small><b>プロフィールを編集</b></div><span></span></header>');

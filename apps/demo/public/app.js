...
function showProfileEditor(){
  ...
  sheet.querySelector('#save-profile').onclick=async()=>{const displayName=sheet.querySelector('#edit-display-name').value.trim();if(!displayName){toast('表示名を入力してください');return}const parsed=sheet.querySelector('#edit-interests').value.split(/[、,]/).map(value=>value.trim()).filter(Boolean);const updatedInterests=[...new Set(parsed)].slice(0,20);try{session.user=await api('/users/me',{method:'PATCH',body:JSON.stringify({displayName,homeArea:sheet.querySelector('#edit-home-area').value.trim()||null,bio:sheet.querySelector('#edit-bio').value.trim()||null,interests:updatedInterests,gender:sheet.querySelector('#edit-gender').value,...(profilePhotosChanged?{profilePhotos:pendingProfilePhotos.filter(Boolean).slice(0,3)}:{})})});saveSession();sheet.remove();await profileScreen();toast('プロフィールを更新しました')}catch(error){toast(error.message)}};
}

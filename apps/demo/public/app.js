...
const MATCH_ACTIVITY_OPTIONS=[...INTEREST_OPTIONS,'チル'];
...
function authScreen(mode = 'login') {
  ...
  const providerSection = `<div class="auth-providers" aria-label="その他の認証方法"><button type="button" data-auth-provider="Google"><span class="provider-mark google-mark">G</span>Google${providerAction}</button><button type="button" data-auth-provider="Apple"><span class="provider-mark apple-mark">●</span>Apple${providerAction}</button><button type="button" data-auth-provider="X"><span class="provider-mark">X</span>X${providerAction}</button><button type="button" data-auth-provider="LINE"><span class="provider-mark line-mark">L</span>LINE${providerAction}</button></div><div id="provider-auth-note" class="provider-auth-note" role="status"></div>`;
  ...
  document.querySelectorAll('[data-auth-provider]').forEach((button)=>button.onclick=async()=>{const provider=button.dataset.authProvider;if(!['Google','Apple','X','LINE'].includes(provider)){document.querySelector('#provider-auth-note').textContent=`${provider}認証は現在利用できません。`;return}const input=null;try{const providerKey=provider.toLowerCase();sessionStorage.setItem('hangout-now-oauth-input','null');const returnTo=`${location.origin}${location.pathname}`;location.assign(`${API_URL}/auth/${providerKey}/start?returnTo=${encodeURIComponent(returnTo)}`)}catch(error){button.disabled=false;document.querySelector('#provider-auth-note').textContent=error.message}});
  document.querySelector('#switch-auth').onclick = () => authScreen(register ? 'login' : 'register');
  document.querySelector('#auth-form').onsubmit = async (event) => { event.preventDefault(); const button=event.submitter; button.disabled=true; button.textContent='接続中…'; const body={email:document.querySelector('#email').value.trim(),password:document.querySelector('#password').value}; try{if(register){Object.assign(body,{displayName:document.querySelector('#display-name').value.trim(),birthDate:document.querySelector('#birth-date').value,gender});const acquisition=pendingAcquisition();if(acquisition)body.acquisition=acquisition;const files=[...document.querySelector('#register-photo').files];if(files.length>3)throw new Error('プロフィール画像は3枚まで選択できます');if(files.length)body.profilePhotos=await Promise.all(files.map(imageData))}session=await api(register?'/auth/register':'/auth/login',{method:'POST',body:JSON.stringify(body)});demoRole=null;localStorage.removeItem(DEMO_ROLE_STORAGE_KEY);saveSession();connectRealtime();await Promise.all([loadNotificationCount(),loadHangouts()]);if(register){clearPendingAcquisition();await profileScreen()}else navigate('home')}catch(error){document.querySelector('#auth-error').textContent=error.message;button.disabled=false;button.textContent=register?'無料で登録':'ログイン'} };
...
async function redeemOAuthTicket(){
  ...
  try{const acquisition=pendingAcquisition();const result=await api(`/auth/${provider}/redeem`,{method:'POST',body:JSON.stringify({ticket,...(input||{}),...(acquisition?{acquisition}:{})})});if(result.registrationRequired){sessionStorage.setItem('hangout-now-oauth-pending',JSON.stringify({provider,ticket,displayName:result.displayName||''}));authScreen('register');const displayName=document.querySelector('#display-name');if(displayName&&!displayName.value)displayName.value=result.displayName||'';const registrationNote=document.querySelector('#provider-auth-note');if(registrationNote)registrationNote.textContent=`表示名と生年月日を入力し、「${providerLabel}でアカウント作成」を押してください。LINE認証をやり直す必要はありません。`;return}session=result;clearPendingAcquisition();demoRole=null;sessionStorage.removeItem('hangout-now-oauth-pending');sessionStorage.removeItem('hangout-now-oauth-input');sessionStorage.removeItem('hangout-now-line-input');localStorage.removeItem(DEMO_ROLE_STORAGE_KEY);saveSession();connectRealtime();await Promise.all([loadNotificationCount(),loadHangouts()]);navigate('home')}catch(error){if(note)note.textContent=error.message}
}
...
async function profileScreen({animate=true}={}) {
  ...
  document.body.insertAdjacentHTML('beforeend',`<div class="profile-screen"><header class="host-menu-header"><button class="brand-back" type="button" aria-label="ホームに戻る"><span></span></button><div><small>アカウント</small><b>プロフィール</b></div><span></span></header><main class="profile-screen-content"><section class="profile">${profilePhotoTrio(session.user,session.user.displayName)}<h1>${safeText(session.user.displayName)}</h1><button class="profile-chat-button" id="profile-chat"><span>●</span>トーク</button><p>${safeText(session.user.bio||'自己紹介を登録しましょう。')}</p><button class="primary profile-edit-button" id="edit-profile">プロフィールを編集</button><div class="stats"><div><b>—</b><span>主催評価</span></div><div><b>—</b><span>参加評価</span></div><div><b>${joined.size}</b><span>参加</span></div><div><b>${hosted}</b><span>主催</span></div></div><h2>興味のあること</h2><div class="tags">${(session.user.interests||[]).map(i=>`<span>${safeText(i)}</span>`).join('')||'<span>未登録</span>'}</div><div class="safety">🛡️ 募集を作るには、顔が分かるプロフィール写真が必要です。</div></section></main></div>`);
...
function showProfileEditor(){
  ...
  genderSelect.insertAdjacentHTML('afterend',`<div class="matching-preferences"><h3>マッチング設定</h3><p class="matching-lead">タップするだけ。複数選べる項目は、もう一度タップすると解除できます。</p><p class="matching-privacy">入力は任意です。位置は市区・駅などのおおまかなエリアだけを保存し、正確なGPS位置は保存しません。</p>
    ...
    <div class="matching-field"><b>希望する活動</b><small>興味があるものを選択</small><div class="match-choice-grid">${MATCH_ACTIVITY_OPTIONS.map(value=>chip('data-match-activity',value,value,(session.user.preferredActivities||[]).includes(value))).join('')}</div><input id="edit-preferred-activities" maxlength="500" placeholder="ほかの活動を追加" value="${safeText((session.user.preferredActivities||[]).filter(value=>!MATCH_ACTIVITY_OPTIONS.includes(value)).join('、'))}"></div>
...
if (session) {connectRealtime();loadNotificationCount();loadHangouts().then(()=>navigate('home')).catch(()=>navigate('home'))} else authScreen(new URL(location.href).searchParams.get('mode')==='register'?'register':'login');

...
function card(h) {
  ...
  return `<article class="card ${requested ? 'requested' : ''}" data-id="${h.id}"><div class="card-status">${state}</div><div class="card-top"><div class="icon">${h.icon}</div><div><h3>${h.title}</h3><div class="meta"><span class="${h.hot ? 'hot' : ''}">${h.time}</span> ・ ${h.place}${h.distanceKm!==null&&h.distanceKm!==undefined?` ・ ${h.distanceKm}km`:''}</div><div class="meta people" style="margin-top:5px">👥 ${h.current} / ${h.max}人 ・ ${conditions} ${h.distanceKm>10?'<b class="far">・遠め</b>':''}</div></div></div><div class="card-bottom"><div class="host"><span class="mini ${portraitClass(h.photo)}"${photoStyle(h.hostPhoto)} aria-label="${h.host}のプロフィール写真"></span><span>${h.host} ${h.verified?'✓':''}<br><b class="host-tier tier-${(h.hostStatus?.tier||'BRONZE').toLowerCase()}">${h.hostStatus?.label||'ブロンズ'}</b> ・ ${h.rating}</span></div><span class="match">相性 ${h.match}%</span></div></article>`;
}

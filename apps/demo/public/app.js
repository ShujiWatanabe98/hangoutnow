...
function refreshCountdowns(){document.querySelectorAll('.card[data-id]').forEach(card=>{const hangout=hangouts.find(item=>String(item.id)===card.dataset.id);const label=card.querySelector('.card-body .meta span');if(hangout&&label)label.textContent=countdownLabel(hangout.startAt)});document.querySelectorAll('.hangout-detail-sheet[data-hangout-id]').forEach(sheet=>{const hangout=hangouts.find(item=>String(item.id)===sheet.dataset.hangoutId);const label=sheet.querySelector('.detail-time');if(hangout&&label)label.textContent=`${countdownLabel(hangout.startAt)} ・ 相性 ${hangout.match}%`})}

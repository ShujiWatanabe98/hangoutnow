...
function authScreen(mode = 'login') {
  ...
  document.querySelectorAll('[data-auth-provider]').forEach((button)=>button.onclick=async()=>{const provider=button.dataset.authProvider;if(!['Google','Apple','X','LINE'].includes(provider)){document.querySelector('#provider-auth-note').textContent=`${provider}認証は公開テスト開始時に利用できます。`;return}const requiresProfileInput=register&&!['LINE','X'].includes(provider);const input=requiresProfileInput?{displayName:document.querySelector('#display-name').value.trim(),birthDate:document.querySelector('#birth-date').value,gender}:null;if(requiresProfileInput&&(!input.displayName||!input.birthDate)){document.querySelector('#provider-auth-note').textContent=`表示名と生年月日を入力してから${provider}登録してください。`;return}try{const files=[...(document.querySelector('#register-photo')?.files||[])];if(files.length>3)throw new Error('プロフィール画像は3枚まで選択できます');if(input&&files.length)input.profilePhotos=await Promise.all(files.map(imageData));const providerKey=provider.toLowerCase();let pending=null;try{pending=JSON.parse(sessionStorage.getItem('hangout-now-oauth-pending')||'null')}catch{pending=null}if(register&&pending?.provider===providerKey&&pending.ticket){button.disabled=true;document.querySelector('#provider-auth-note').textContent=`${provider}アカウントを作成しています…`;const result=await api(`/auth/${providerKey}/redeem`,{method:'POST',body:JSON.stringify({ticket:pending.ticket,...input})});session=result;demoRole=null;sessionStorage.removeItem('hangout-now-oauth-pending');sessionStorage.removeItem('hangout-now-oauth-input');localStorage.removeItem(DEMO_ROLE_STORAGE_KEY);saveSession();connectRealtime();await Promise.all([loadNotificationCount(),loadHangouts()]);await profileScreen();return}sessionStorage.setItem('hangout-now-oauth-input',JSON.stringify(input));const returnTo=`${location.origin}${location.pathname}`;location.assign(`${API_URL}/auth/${providerKey}/start?returnTo=${encodeURIComponent(returnTo)}`)}catch(error){button.disabled=false;document.querySelector('#provider-auth-note').textContent=error.message}});

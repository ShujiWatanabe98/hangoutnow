...
function card(h) {
  ...
  return `<article class="card ${requested ? 'requested' : ''}" data-id="${h.id}"><div class="card-status">${state}</div><div class="card-top"><div class="icon">${h.icon}</div><div><h3>${h.title}</h3><div class="meta"><span class="${h.hot ? 'hot' : ''}">${h.time}</span> ・ ${h.place}${h.distanceKm!==null&&h.distanceKm!==undefined?` ・ ${h.distanceKm}km`:''}</div><div class="meta people" style="margin-top:5px">👥 ${h.current} / ${h.max}人 ・ ${conditions} ${h.distanceKm>10?'<b class="far">・遠め</b>':''}</div></div></div><div class="card-bottom"><div class="host"><span class="mini ${portraitClass(h.photo)}"${photoStyle(h.hostPhoto)} aria-label="${h.host}のプロフィール写真"></span><span>${h.host} ${h.verified?'✓':''}<br><b class="host-tier tier-${(h.hostStatus?.tier||'WHITE').toLowerCase()}">${h.hostStatus?.label||'ホワイト'}</b> ・ ${h.rating}</span></div><span class="match">相性 ${h.match}%</span></div></article>`;
}
...
async function chatScreen() {
  ...
  const demoChatHint=demoRole?`<div class="demo-chat-hint"><span>${demoRole==='host'?'会話後「Hangoutを終了」→参加者を★1〜5で評価':'終了後に主催者を★1〜5で評価（双方★5なら1対1）'}</span><button id="chat-switch-demo-role">役割切替</button></div>`:'';
  shell(`<section class="chat-workspace"><aside class="chat-sidebar"><section class="page-title chat-page-title"><div><div class="eyebrow">会話から次の行動へ</div><h1>チャット</h1></div><span class="realtime-pill">● リアルタイム</span></section>${demoChatHint}<div class="chat-tabs"><button class="active" data-chat-tab="group">グループ <b>${groups.length}</b></button><button data-chat-tab="direct">1対1 <b>${directs.length}</b></button></div><div class="chat-lists"><section class="chat-list" data-chat-list="group">${groupRows||'<div class="empty">参加が承認されるとグループチャットが表示されます。</div>'}</section><section class="chat-list hidden" data-chat-list="direct">${directRows||'<div class="empty">双方の★5評価がそろうと1対1チャットが表示されます。</div>'}</section></div></aside><section class="chat-conversation" id="chat-conversation"><div class="conversation-placeholder"><span>♡</span><b>会話を選んでください</b><small>相手の表情を感じながら、待ち合わせまで自然に話せます。</small></div></section></section>`, false);
  ...
  document.querySelectorAll('[data-finish-hangout]').forEach((button)=>button.onclick=async()=>{try{await api(`/hangouts/${button.dataset.finishHangout}/finish`,{method:'POST'});toast('Hangoutを終了しました。参加者を評価できます');chatScreen()}catch(error){toast(error.message)}});
...
async function openChat(room) {
  ...
  const canFinish=kind==='group'&&room.hangout.host.id===session.user.id&&room.hangout.status!=='FINISHED';
  conversation.innerHTML=`<header class="chat-conversation-head"><button class="chat-back" type="button" aria-label="チャット一覧に戻る">＜</button><span class="conversation-avatar"${photoStyle(photo)}>${photo?'':safeText(title).slice(0,1)}</span><div><h2>${safeText(title)}</h2><small>${kind==='direct'?'1対1':room.hangout.status==='FINISHED'?'終了・評価待ち':'グループ ・ '+room.members.length+'人'}　<span>● オンライン</span></small></div>${canFinish?'<button class="conversation-finish">終了</button>':''}</header><div class="messages">${messages.map((m)=>{const mine=m.senderUserId===session.user.id;return `<div class="message-line ${mine?'mine':''}">${mine?'':`<span class="message-avatar"${photoStyle(m.sender.profilePhoto)}>${m.sender.profilePhoto?'':safeText(m.sender.displayName).slice(0,1)}</span>`}<div class="bubble-wrap"><small>${safeText(m.sender.displayName)}</small><div class="bubble">${messageContent(m.body)}</div><time>${new Date(m.createdAt).toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'})}</time></div>${mine?`<span class="message-avatar"${photoStyle(m.sender.profilePhoto||session.user.profilePhoto)}>${m.sender.profilePhoto||session.user.profilePhoto?'':safeText(m.sender.displayName).slice(0,1)}</span>`:''}</div>`}).join('')||'<div class="empty">最初のメッセージを送ってみましょう。</div>'}</div><div class="conversation-actions"><div class="stamp-tray">${stamps.map(stamp=>`<button data-stamp="${stamp.id}" class="stamp-choice"><img src="${safeText(stamp.imageUrl)}" alt="${safeText(stamp.text)}"><b>${safeText(stamp.text)}</b></button>`).join('')}<button class="stamp-create">＋<small>スタンプ作成</small></button></div>${kind==='group'?'<div class="quick"><button>向かっています</button><button>少し遅れます</button><button>到着しました</button></div>':''}<div class="composer"><input placeholder="メッセージを入力" maxlength="1000"><button>送信</button></div></div>`;
  ...
  const finishButton=conversation.querySelector('.conversation-finish');if(finishButton)finishButton.onclick=()=>showFinishConfirmation(room);
